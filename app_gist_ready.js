(function(){
    ...
    window.onerror = function(msg, src, line, col, err) {
      showLoadErr('Error: ' + msg + (err && err.stack ? '\n' + err.stack : ''));
      return true;
    };
...
if (window.AXIS_DEV) {
  try {
    ...
    _dbg.style = 'position:fixed;right:12px;bottom:12px;z-index:99999;background:rgba(0,0,0,0.6);color:#fff;padding:8px 10px;border-radius:8px;font-family:monospace;font-size:12px;cursor:pointer';
...
/* Onboarding moved to /public./onboarding/ — inline implementation removed */
// ─────────────────────────────────────────────────────────────
//  TRACK DATA
// ─────────────────────────────────────────────────────────────
...
const DAILY_SECTIONS = [
  {
    ...
    exercises: [
      { id: 1, name: "Cat–Cow", start: "On all fours — hands under shoulders, knees under hips.", sub: "Segmental Spinal Flexion/Extension", type: "flow", seconds: 45, demoVideo: "/assets/videos/cat_cow.mp4", steps: [
  ...
  },
  {
    ...
    exercises: [
      { id: 5, name: "Pelvic Tilts", start: "Lying on your back, knees bent, feet flat on the floor.", sub: "Supine Posterior/Anterior Tilt", reps: "10–12 reps", type: "reps", steps: [
        { pos: "A", label: "Supine Neutral", focus: "SETUP", cue: "Small natural arch in lower back. Arms at sides." },
        ...
        { pos: "C", label: "Anterior Tilt", focus: "RELEASE", cue: "Let the lower back arch slightly away from the floor as the pelvis tips forward. Then gently press the lower back into the floor again. Move slowly back and forth between the small arch and the gentle press." },
      ]},
      { id: 6, name: "Glute Bridge", start: "Lying on your back, knees bent, feet flat on the floor.", sub: "Bodyweight", reps: "12–15 reps", type: "reps", steps: [
        { pos: "A", label: "Supine Setup", focus: "SETUP", cue: "Feet flat, hip-width, heels 6 inches from glutes. Arms flat at sides." },
      ...
      { id: 8, name: "Dead Bug", start: "Lying on your back, arms reaching toward the ceiling.", sub: "Contralateral Arm/Leg", reps: "6–8 per side, slow", type: "reps", steps: [
        { pos: "A", label: "Supine Tabletop", focus: "SETUP", cue: "Arms reach to ceiling. Hips and knees at 90°. Press lower back firmly into floor." },
  ...
  },
  {
    ...
    exercises: [
      { id: 15, name: "Supine Hamstring Stretch", start: "Lying on your back, both legs extended. You'll do one leg at a time (e.g. right first, then left).", sub: "Straight-Leg Raise", type: "flow", seconds: 45, steps: [
        { pos: "A", label: "Supine Start", focus: "SETUP", cue: "Lie on your back. Both legs extended, lower back neutral. Arms relaxed at your sides." },
        { pos: "B", label: "Raise One Leg", focus: "STRETCH", cue: "Lift one leg (e.g. right first) keeping the knee straight. Use both hands to hold behind the thigh or calf. Feel the stretch in the hamstring, not the knee. Switch to the other leg when cued." },
      ]},
      { id: 16, name: "Figure Four Stretch", start: "Lying on your back, knees bent, feet flat. Cross one ankle over the opposite knee (e.g. right ankle over left knee); you'll do both sides.", sub: "Supine Piriformis Stretch", type: "flow", seconds: 45, steps: [
        { pos: "A", label: "Cross Leg Over Knee", focus: "SETUP", cue: "Lie on your back, knees bent, feet flat. Cross one leg over the other by placing that ankle on the opposite knee (e.g. right ankle on left knee). Flex the foot of the crossed leg." },
        { pos: "B", label: "Draw In", focus: "STRETCH", cue: "Clasp hands behind the supporting thigh (the one with the foot on the floor) and gently draw both legs toward your chest. Feel the stretch in the outer hip of the crossed leg. Repeat on the other side when cued." },
      ]},
      { id: 17, name: "Windshield Wipers", start: "Lying on your back, knees bent, arms out in a T. Rotate knees side to side.", sub: "Supine Hip Rotation", type: "flow", seconds: 45, steps: [
        { pos: "A", label: "Supine, Feet Flat", focus: "SETUP", cue: "Lie on your back. Knees bent, feet flat and hip-width. Arms out in a T, palms down for stability." },
        ...
        { pos: "C", label: "Lower Knees Left", focus: "ROTATE", cue: "Sweep knees back through center and lower to the left. Move at a slow, windshield-wiper pace. Continue alternating as needed." },
      ]},
      { id: 18, name: "Reclined Spinal Twist", start: "Lying on your back, one knee drawn to your chest. Do one side (e.g. right) then the other.", sub: "Arms in T position", type: "flow", seconds: 45, steps: [
        { pos: "A", label: "Supine Start", focus: "SETUP", cue: "Lie on your back. Draw one knee to your chest (e.g. right first). Arms out in a T, shoulders heavy on the floor." },
        { pos: "B", label: "Cross & Hold", focus: "TWIST", cue: "Guide that knee across your body with the opposite hand (e.g. right knee to the left with left hand). Turn gaze the other way if comfortable. Hold 20–30s. Repeat on the other side when cued." },
      ]},
    ],
  },
  {
...
const MORNING_SECTIONS = [
  {
    ...
    exercises: [
      ...
      { id: 203, name: "Cat–Cow", start: "On all fours — hands under shoulders, knees under hips.", sub: "Segmental spinal warm-up", type: "flow", seconds: 45, demoVideo: "/assets/videos/cat_cow.mp4", steps: [
...
const PRIME_SECTIONS = [
  {
    ...
    exercises: [
      ...
      { id: 302, name: "Reclined Butterfly", start: "Lying on your back, soles of feet together.", sub: "Supine bound angle, hips open", type: "flow", seconds: 90, steps: [
...
const BACK_SECTIONS = [
  {
    ...
    exercises: [
      ...
      { id: 702, name: "Supine Pelvic Tilts", start: "Lying on your back, knees bent, feet flat.", sub: "Supine posterior/anterior tilt", type: "reps", reps: "10–12 reps", steps: [
        { pos: "A", label: "Lie on Back", focus: "SETUP", cue: "Small natural arch in lower back — this is neutral. Arms flat at sides. Do not flatten the back before you begin." },
      ...
      { id: 703, name: "Cat–Cow", start: "On all fours — hands under shoulders, knees under hips.", sub: "Slow and segmental, 10 cycles", type: "flow", seconds: 45, demoVideo: "/assets/videos/cat_cow.mp4", steps: [
        { pos: "A", label: "Neutral Tabletop", focus: "SETUP", cue: "Spine neutral. Take one full breath here before moving. This is the most therapeutic movement for the lumbar spine." },
      ...
      { id: 705, name: "Windshield Wipers", start: "Lying on your back, knees bent, arms in a T.", sub: "Slow rotation, 10 reps each", type: "flow", seconds: 45, steps: [
        { pos: "A", label: "Supine Setup", focus: "SETUP", cue: "Knees bent, feet flat and hip-width. Arms in a T, palms down. Shoulders stay grounded throughout." },
...
const ANXIETY_SECTIONS = [
  {
    ...
    exercises: [
      { id: 2001, name: "Cold Shock (Dive Reflex)", start: "Standing or seated near a sink or bowl of cold water.", sub: "Rapid parasympathetic activation via the mammalian dive reflex", type: "time", seconds: 30, steps: [
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Hold a golf club across your upper back and shoulders, gripping each end. Keep your spine straight and knees slightly bent." },
          { focus: "ROTATE", cue: "Rotate your torso to the right as far as comfortable while keeping your hips and feet facing forward." },
          { focus: "SWITCH", cue: "Slowly rotate to the left, feeling the separation between your upper and lower body." },
        ],
      },
      {
        id: 3002,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Stand on one leg, engage your core, and rest a hand lightly on the wall or cart for balance." },
          { focus: "SWING", cue: "Swing the opposite leg forward and backward in a controlled motion, gradually increasing the range as the hip warms up." },
          { focus: "LATERAL", cue: "Switch to side-to-side swings in front of your body to open the hip socket in the lateral plane." },
        ],
      },
      {
        id: 3003,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Make light fists with both hands with arms extended at shoulder height." },
          { focus: "CIRCLE", cue: "Rotate your wrists in slow, controlled clockwise circles, using the full available range." },
          { focus: "REVERSE", cue: "Change direction to counter-clockwise, releasing tension through the forearms." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SPINE", cue: "Extend both arms straight out in front of you with palms touching and knees stacked." },
          { focus: "OPEN", cue: "Lift the top arm and rotate it slowly across your body toward the floor behind you while keeping your knees pinned together." },
          { focus: "RESET", cue: "Breathe deeply into the stretch, then slowly return to the starting position and repeat." },
        ],
      },
      {
        id: 3005,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Keep your front foot flat, torso upright, and ribs stacked over the pelvis." },
          { focus: "TUCK", cue: "Gently tuck your pelvis under and lean forward slightly until you feel a stretch in the front of your back hip." },
          { focus: "REACH", cue: "Reach the arm on the same side as your back knee toward the ceiling to deepen the release." },
        ],
      },
      {
        id: 3006,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Slide one arm underneath the other, palm up, reaching as far as comfortable." },
          { focus: "ROTATE", cue: "Lower your shoulder and ear toward the floor, letting the upper back rotate." },
          { focus: "BREATH", cue: "Hold and breathe into the space between your shoulder blades before switching sides." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Shift your weight to the right, bending the right knee while keeping the left leg straight." },
          { focus: "REACH", cue: "Reach your arms forward to balance your weight and keep the chest lifted." },
          { focus: "SWITCH", cue: "Push off the right foot to return to center, then repeat on the left side." },
        ],
      },
      {
        id: 3012,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Let your arms hang down with thumbs pointing out and spine long." },
          { focus: "Y-RAISE", cue: "Raise your arms up to form a Y shape, squeezing the shoulder blades together." },
          { focus: "T-RAISE", cue: "Lower your arms, then raise them straight out to the sides to form a T, maintaining control." },
        ],
      },
      {
        id: 3013,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Stay light on the balls of your feet with knees slightly soft." },
          { focus: "HOP", cue: "Perform small, quick vertical hops, focusing on a fast bounce off the ground." },
          { focus: "STABILITY", cue: "Keep your core tight and land softly each time without letting the heels slam down." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Point your fingers toward the floor with the palm facing you." },
          { focus: "STRETCH", cue: "Use the opposite hand to gently pull the back of your hand toward your body until you feel a stretch." },
          { focus: "RELEASE", cue: "Hold for several breaths, then ease off and switch sides." },
        ],
      },
      {
        id: 3015,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Cross your right ankle over your left knee to create a figure-4 shape." },
          { focus: "PULL", cue: "Reach through and grab your left thigh, gently pulling it toward your chest." },
          { focus: "SWITCH", cue: "Hold for several breaths, then repeat on the opposite side." },
        ],
      },
      {
        id: 3016,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Fold forward and rest your forehead on the floor with arms extended in front of you." },
          { focus: "REACH", cue: "Walk both hands to the right until you feel a stretch along your left side and lat." },
          { focus: "SWITCH", cue: "Walk hands back to center, then over to the left side and repeat." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Lift one foot off the ground while lightly supporting yourself on the wall if needed." },
          { focus: "CIRCLE", cue: "Rotate the ankle in slow, controlled clockwise circles." },
          { focus: "REVERSE", cue: "Switch to counter-clockwise circles, then repeat with the other foot." },
        ],
      },
      {
        id: 3022,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Shift weight to your left leg and engage your core." },
          { focus: "DRIVE", cue: "Lift the right knee toward your chest while squeezing the left glute." },
          { focus: "SWITCH", cue: "Lower the foot with control and repeat on the other side, staying tall." },
        ],
      },
      {
        id: 3023,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Bend your elbows slightly and let the arms hang by your sides." },
          { focus: "SWING", cue: "Swing your arms forward and backward from the shoulder joint in a natural rhythm." },
          { focus: "RELAX", cue: "Keep your neck long and avoid shrugging the shoulders as you move." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Place both hands on the wall and step one foot back." },
          { focus: "PRESS", cue: "Keep the back heel on the floor and lean into the wall until you feel a stretch in the calf." },
          { focus: "BEND", cue: "Slightly bend the back knee to target the lower calf and Achilles, then switch sides." },
        ],
      },
      {
        id: 3025,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Reach back and grab your right ankle or foot." },
          { focus: "ALIGN", cue: "Pull the heel toward your glute while keeping your knees together and torso upright." },
          { focus: "SWITCH", cue: "Hold the stretch, then repeat on the left side." },
        ],
      },
      {
        id: 3026,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Place a small ball or firm object under the arch of your foot." },
          { focus: "ROLL", cue: "Apply gentle pressure and roll the foot forward and backward over the object." },
          { focus: "FOCUS", cue: "Spend extra time on any tight or tender spots before switching feet." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Keep your knees almost straight and stand tall." },
          { focus: "BOUNCE", cue: "Jump using only your ankles and calves, staying on the balls of your feet." },
          { focus: "RHYTHM", cue: "Maintain a quick, rhythmic pace to prime the tendons." },
        ],
      },
      {
        id: 3032,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Step back into a lunge, keeping the back leg straight and front knee stacked over the ankle." },
          { focus: "REACH", cue: "Reach both arms toward the ceiling and tilt your torso slightly away from the back leg." },
          { focus: "SWITCH", cue: "Return to standing and switch sides with control." },
        ],
      },
      {
        id: 3033,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Hinge at the hips while keeping the front leg straight and back knee soft." },
          { focus: "SCOOP", cue: "Reach down and scoop your hands toward the floor in front of the foot, then up overhead." },
          { focus: "SWITCH", cue: "Take a small step forward and repeat on the other leg." },
        ],
      },
    ],
  },
    ...
    exercises: [
      {
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Bend your left knee and cross your right ankle over the left thigh." },
          { focus: "PRESS", cue: "Lean forward slightly or pull the left thigh toward you to feel the stretch in the right hip." },
          { focus: "SWITCH", cue: "Hold, then repeat on the opposite side." },
        ],
      },
      {
        id: 3035,
        ...
        seconds: 45,
        steps: [
          { focus: "SETUP", cue: "Place your hands on the floor next to your shoulders with elbows close to your sides." },
          { focus: "LIFT", cue: "Gently press through your hands to lift your chest off the floor, keeping your hips grounded." },
          { focus: "EXTEND", cue: "Draw shoulders away from your ears and breathe into the front of the body." },
        ],
      },
      {
        id: 3036,
        ...
        seconds: 120,
        steps: [
          { focus: "SETUP", cue: "Extend your legs straight up against the wall with heels resting lightly." },
          { focus: "RELAX", cue: "Let your arms rest by your sides with palms up and jaw unclenched." },
          { focus: "BREATHE", cue: "Stay here for about two minutes, breathing slowly to calm the system." },
        ],
      },
    ],
  },
...
const THORACIC_SECTIONS = [
  {
    ...
    exercises: [
      { id: 2000, name: "Thoracic Extension (foam roller)", start: "Seated or supine with foam roller/rolled towel under the upper back (shoulder blade level).", sub: "Extension options", type: "time", seconds: 45, steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "Place foam roller or rolled towel under upper back. Keep knees bent and feet on the floor if lying." },
        { pos: "B", label: "Extend", focus: "EXTEND", cue: "Support your head with hands and gently arch back over the roller. Pause and breathe for 3-5 breaths." },
        { pos: "C", label: "Alternative", focus: "VARIATION", cue: "If no roller, use a chair back, wall, or puppy-pose quadruped variation to find comfortable extension." },
      ]},
      { id: 2001, name: "Cat–Cow", start: "On all fours, neutral spine.", sub: "Segmental flexion/extension", type: "reps", reps: "10 reps", demoVideo: "/assets/videos/cat_cow.mp4", steps: [
        { pos: "A", label: "Neutral Tabletop", focus: "SETUP", cue: "Hands under shoulders, knees under hips. Neutral spine, neck long." },
        { pos: "B", label: "Cow — Inhale", focus: "EXTEND", cue: "Inhale, drop the belly, lift the chest and tailbone." },
        { pos: "C", label: "Cat — Exhale", focus: "FLEX", cue: "Exhale, round through the spine segment by segment. Move slowly for 10 cycles." },
      ]},
      { id: 2002, name: "Thoracic Rotation", start: "Seated or kneeling with arms crossed.", sub: "Rotation, 5–10 per side", type: "reps", reps: "5–10 per side", steps: [
        { pos: "A", label: "Seated Setup", focus: "SETUP", cue: "Sit tall with hands across chest or arms hugging shoulders." },
        { pos: "B", label: "Rotate", focus: "ROTATE", cue: "Rotate upper body gently to one side, keeping hips facing forward. Return and repeat other side." },
      ]},
      { id: 2003, name: "Wall Angels", start: "Standing with back against a wall.", sub: "Scapular retraction & mobility", type: "reps", reps: "5–10 reps", steps: [
        { pos: "A", label: "Wall Contact", focus: "SETUP", cue: "Head, upper back and glutes touch the wall. Arms in a W at shoulder height." },
        { pos: "B", label: "Slide to Y", focus: "ACTIVATE", cue: "Slide arms up overhead into a Y keeping contact with the wall. Return slowly." },
      ]},
      { id: 2004, name: "Chin Tucks", start: "Seated or standing tall.", sub: "Cervical retraction", type: "reps", reps: "10 reps", steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "Sit or stand tall, eyes forward." },
        { pos: "B", label: "Tuck", focus: "NECK", cue: "Draw chin straight back (not down), hold gently for 3–5 seconds then release." },
      ]},
      { id: 2005, name: "Thread the Needle", start: "All fours — hands under shoulders, knees under hips.", sub: "Thoracic rotation & shoulder release", type: "time", seconds: 25, steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "From tabletop, reach one arm under the body and rest shoulder on the floor." },
        { pos: "B", label: "Hold", focus: "RELEASE", cue: "Breathe here 20–30 seconds per side and feel rotation through the upper back." },
      ]},
      { id: 2006, name: "Child's Pose", start: "Kneel and reach arms forward.", sub: "Gentle spine flexion & reset", type: "time", seconds: 25, steps: [
        { pos: "A", label: "Kneel & Fold", focus: "SETUP", cue: "Sit back toward heels and fold forward with arms extended. Relax the chest toward the floor." },
      ]},
    ],
  },
];

const WRIST_SECTIONS = [
  {
    ...
    exercises: [
      { id: 2010, name: "Wrist Flexor Stretch", start: "Seated or standing, arm extended.", sub: "Stretch the palm-side forearm", type: "time", seconds: 30, steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "Extend the arm with palm facing up." },
        { pos: "B", label: "Stretch", focus: "STRETCH", cue: "Use the other hand to gently pull fingers back toward you. Hold 30s per side." },
      ]},
      { id: 2011, name: "Wrist Extensor Stretch", start: "Arm extended palm down.", sub: "Stretch the top forearm", type: "time", seconds: 30, steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "Extend the arm with palm facing down." },
        { pos: "B", label: "Stretch", focus: "STRETCH", cue: "Use the other hand to press the fingers toward the forearm. Hold 30s per side." },
      ]},
      { id: 2012, name: "Median Nerve Glide", start: "Seated, arm supported.", sub: "Nerve mobility", type: "reps", reps: "10 reps", steps: [
        { pos: "A", label: "Setup", focus: "SETUP", cue: "Start with a neutral wrist and fingers." },
        { pos: "B", label: "Glide", focus: "GLIDE", cue: "Make a fist, then straighten fingers and thumb across the palm. Repeat smoothly for 10 reps." },
      ]},
      { id: 2013, name: "Prayer Stretch", start: "Standing or seated at chest height.", sub: "Gentle wrist flexion", type: "time", seconds: 20, steps: [
        { pos: "A", label: "Prayer Setup", focus: "SETUP", cue: "Bring palms together at chest, fingers pointing up." },
        { pos: "B", label: "Lower", focus: "STRETCH", cue: "Slowly lower hands while keeping palms together to stretch the wrists. Hold 20s." },
      ]},
      { id: 2014, name: "Shakeout & Fist Pumps", start: "Standing or seated.", sub: "Loosen the hands and forearms", type: "time", seconds: 30, steps: [
        { pos: "A", label: "Shake", focus: "RELEASE", cue: "Shake both hands loose for 15 seconds." },
        { pos: "B", label: "Fist Pumps", focus: "ACTIVATE", cue: "Alternate making a fist and opening the hand with punch-like pumps for 15 seconds." },
      ]},
    ],
  },
];

const TRACKS = (window.AXIS_JSON && window.AXIS_JSON.TRACKS) ? window.AXIS_JSON.TRACKS : {
  daily:       { id: "daily",       label: "The Baseline Protocol", subtitle: "Spine, Core, Posterior", purpose: "Spine, core, and posterior chain. The foundation session.", sections: DAILY_SECTIONS, duration: "25 min" },
  restorative: { id: "restorative", label: "Lymph & Flow", subtitle: "Circulation, Mobility, Recovery", purpose: "Gentle continuous movement for circulation, joint lubrication, and recovery.", sections: RESTORATIVE_SECTIONS, duration: "15 min" },
  morning:     { id: "morning",     label: "Sunrise Stretch", subtitle: "Wake, Spine, Energy", purpose: "Seven movements to wake the spine before you look at your phone.", sections: MORNING_SECTIONS, duration: "8 min" },
  prime:       { id: "prime",       label: "Night Ease", subtitle: "Decompress, Calm, Sleep", purpose: "Lower cortisol, decompress the spine, prepare for sleep.", sections: PRIME_SECTIONS, duration: "12 min" },
  desk:        { id: "desk",        label: "Desk Undo", subtitle: "Posture, Sitting, Refresh", purpose: "Six movements to undo an hour of sitting. Stand up. Do it now.", sections: DESK_SECTIONS, duration: "5 min" },
  screen:      { id: "screen",      label: "Tech Neck Release", subtitle: "Neck, Shoulder, Screen", purpose: "Undo the forward pull of screen time. Seven targeted releases for the neck and shoulder girdle.", sections: SCREEN_SECTIONS, duration: "12 min" },
  hip:         { id: "hip",         label: "Hip Freedom", subtitle: "Open, Mobilize, Rotators", purpose: "Open the hips, free the lower back, restore the range that sitting steals.", sections: HIP_SECTIONS, duration: "18 min" },
  back:        { id: "back",        label: "Lumbar Relief", subtitle: "Decompress, Release, Strengthen", purpose: "For days when the lower back needs care, not challenge. Gentle traction and release.", sections: BACK_SECTIONS, duration: "15 min" },
  steady:      { id: "steady",      label: "Stay Steady", subtitle: "Stability, Balance, Coordination", purpose: "Train the systems that prevent falls. Simple movements that are deceptively hard to do well.", sections: STEADY_SECTIONS, duration: "15 min" },
  freely:      { id: "freely",      label: "Joint Glide", subtitle: "CARS, Mobility, Freedom", purpose: "Controlled Articular Rotations and mobility work for every major joint.", sections: FREELY_SECTIONS, duration: "20 min" },
  ...
  travel:      { id: "travel",      label: "Arrival Unwind", subtitle: "Travel, Decompress, Reset", purpose: "Six movements to undo the posture of travel. Do these as soon as you arrive.", sections: TRANSIT_SECTIONS, duration: "15 min" },
  walking:     { id: "walking",     label: "Walk Ease", subtitle: "Pre-Walk, Post-Walk, Reset", purpose: "Wake the gait system before the miles, then decompress calves, quads, and arches after.", sections: WALKING_SECTIONS, duration: "8–10 min" },
  running:     { id: "running",     label: "Run Reboot", subtitle: "Impact, Hips, Recovery", purpose: "Prime tendons and hips before impact, then restore structure and circulation post-run.", sections: RUNNING_SECTIONS, duration: "8–12 min" },
  golf:        { id: "golf",        label: "Swing Sync", subtitle: "Golf, Rotation, Ready", purpose: "Prepare rotation, wrists, and hips before the first tee, then decompress after the final putt.", sections: GOLF_SECTIONS, duration: "8–10 min" },
  tennis:      { id: "tennis",      label: "Court Prep", subtitle: "Racket, Shoulders, Hips", purpose: "Ready the lateral base and shoulders for court work, then release forearms, hips, and side body after play.", sections: TENNIS_SECTIONS, duration: "8–10 min" },
  thoracic:    { id: "thoracic",    label: "Thoracic Chest Reset", subtitle: "Mobility, Upper Back, Chest", purpose: "A guided 7–10 minute flow to restore thoracic mobility and reduce upper back tension.", sections: THORACIC_SECTIONS, duration: "8–10 min" },
  wrist:       { id: "wrist",       label: "Carpal Tunnel Release", subtitle: "Glides, Stretches, Nerves", purpose: "Short PT-style routine for wrist and hand mobility — perfect for keyboard users.", sections: WRIST_SECTIONS, duration: "5–7 min" },
  anxiety:     { id: "anxiety",    label: "Calm Circuit", subtitle: "Breathe, Ground, Focus", purpose: "Evidence-based techniques that directly shift the nervous system from fight-or-flight to rest and digest.", sections: ANXIETY_SECTIONS, duration: "15 min" },
  stress:      { id: "stress",     label: "Tension Release", subtitle: "Stress, Relax, Unwind", purpose: "Complete the stress cycle the body started. Move the cortisol through. Then downshift.", sections: STRESS_SECTIONS, duration: "18 min" },
  feet:        { id: "feet",        label: "Feet First", subtitle: "Fascia, Load, Release", purpose: "PT-backed protocol for plantar fasciitis and foot pain. Release the tissue, wake the muscles, load the chain.", sections: FEET_SECTIONS, duration: "18 min" },
  knee:        { id: "knee",        label: "Knee Resilience", subtitle: "Knee, Mobility, Strength", purpose: "Bulletproof the joint: strengthen the muscles that support it and use low-load movements to lubricate and offload pressure.", sections: KNEE_SECTIONS, duration: "18 min" },
  walking:     { id: "walking",     label: "Walk Ease", subtitle: "Pre-Walk, Post-Walk, Reset", purpose: "Wake the gait system before the miles, then decompress calves, quads, and arches after.", sections: WALKING_SECTIONS, duration: "8–10 min" },
  running:     { id: "running",     label: "Run Reboot", subtitle: "Impact, Hips, Recovery", purpose: "Prime tendons and hips before impact, then restore structure and circulation post-run.", sections: RUNNING_SECTIONS, duration: "8–12 min" },
  golf:        { id: "golf",        label: "Swing Sync", subtitle: "Golf, Rotation, Ready", purpose: "Prepare rotation, wrists, and hips before the first tee, then decompress after the final putt.", sections: GOLF_SECTIONS, duration: "8–10 min" },
  tennis:      { id: "tennis",      label: "Court Prep", subtitle: "Racket, Shoulders, Hips", purpose: "Ready the lateral base and shoulders for court work, then release forearms, hips, and side body after play.", sections: TENNIS_SECTIONS, duration: "8–10 min" },
...
// Resolve lazy section references (avoid TDZ issues)
try {
  if (TRACKS && TRACKS.thoracic && !TRACKS.thoracic.sections) TRACKS.thoracic.sections = THORACIC_SECTIONS;
  if (TRACKS && TRACKS.wrist && !TRACKS.wrist.sections) TRACKS.wrist.sections = WRIST_SECTIONS;
} catch (e) {}
...
const css = `
  ...
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  :root, [data-theme="dark"], [data-theme="light"] {
    ...
    /* canonical app colors */
    --app-white: #f6f7f8;
    --app-black: #252525;
  }
  /* Onboarding typography scale (uniform across slides) */
  :root {
    --ob-heading-size: 32px;
    --ob-subhead-size: 18px;
    --ob-body-size: 15px;
    --ob-eyebrow-size: 12px;
    /* Global app typography to align with onboarding sizes */
    --global-heading-size: 32px;
    --global-subhead-size: 18px;
    --global-body-size: 15px;
    --global-eyebrow-size: 12px;
  }
  /* Enforce consistent onboarding sizes (override inline styles if present) */
  .ob-heading { font-size: var(--ob-heading-size) !important; font-weight: 700 !important; }
  .ob-subhead, .ob-cta-head { font-size: var(--ob-subhead-size) !important; }
  .ob-body { font-size: var(--ob-body-size) !important; line-height:1.28 !important; }
  ...
  .app-body, .app-shell, .app-content { flex: 1; min-height: 0; overflow-y: auto; -webkit-overflow-scrolling: touch; }
  ...
  [data-night="true"] .glass-card::before { background: rgba(255,59,48,0.12) !important; }
  [data-night="true"] .tab-bar .tab-bar-top-line { background: linear-gradient(90deg, transparent, rgba(255,59,48,0.15) 50%, transparent) !important; }
  ...
  .app-body { flex:1; display:flex; flex-direction:column; width:100%; position:relative; min-height:0; }
  ...
  .hdr-title { font-size:24px; font-weight:500; letter-spacing:-0.02em; line-height:1.2; color:var(--text-white); }
  ...
  /* ── CONTENT ── */
  .content { padding:0 20px 24px; max-width:100%; position:relative; z-index:1; }
  .sg {
    margin-top:32px;
    background:var(--glass-bg);
  ...
  .purpose-text { font-size:16px; font-style:italic; line-height:1.5; color:var(--text-secondary); }
  ...
  .ra-btn {
    min-width:30px; height:30px; border-radius:999px; border:1px solid var(--glass-border);
    padding:0 10px;
  ...
  .fc-name { font-size:28px; font-weight:500; letter-spacing:-0.03em; line-height:1.15; margin:4px 0 6px; color:var(--text-white); }
  ...
  .fc-pcue { font-size:18px; color:var(--text-primary); line-height:1.45; font-weight:300; }
  ...
  .fv-name { font-size:40px; font-weight:500; letter-spacing:-0.03em; line-height:1.1; color:var(--text-white); margin-bottom:16px; }
  .fv-cue { font-size:18px; color:var(--text-secondary); font-style:italic; line-height:1.45; }
  ...
  /* ── EXERCISE SUBTITLE ── */
  .es { font-size:14px; color:var(--text-secondary); margin-top:4px; font-style:italic; line-height:1.4; }
  ...
  .step-cue { font-size:19px; color:var(--text-secondary); margin-top:12px; line-height:1.45; }
  ...
  .reps-str {
    font-size:24px; font-weight:500; color:var(--text-white); letter-spacing:-0.01em; line-height:1.25;
  }
  .reps-unit {
  ...
  .momentum-disclaimer { font-size:11px; color:var(--text-dimmer); letter-spacing:0.06em; line-height:1.4; }
  ...
  .tab-bar[data-theme="dark"] {
    --tab-bg: rgba(8,12,20,0.94);
  }
  [data-theme="light"] .tab-bar,
  .tab-bar[data-theme="light"] {
    --tab-bg: rgba(248,250,252,0.92);
  }
  .tab-btn {
  ...
  .track-card-sub { font-size:14px; color:var(--accent-secondary); letter-spacing:0.06em; text-transform:uppercase; margin-bottom:6px; line-height:1.4; }
  .track-card-purpose { font-size:15px; color:var(--text-secondary); font-style:italic; line-height:1.45; margin-top:10px; }
...
function Timer({ seconds, onDone, nextName }) {
  ...
  useEffect(() => {
    ...
    else { clearInterval(ref.current); if (left === 0 && running) { setRunning(false); onDone?.(); } }
  ...
  return (
    <div className="timer-wrap">
      <div className="timer-top">
        <div className="timer-val">{display}</div>
      </div>
      <div className="timer-btns">
        <button className="tb" onClick={() => setRunning(r => !r)}>{label}</button>
        <button className="tb-g" onClick={() => { setLeft(seconds); setRunning(false); clearInterval(ref.current); }}>Reset</button>
      </div>
    </div>
  );
}
...
function RestTimer({ onDone }) {
  ...
  return (
    <div className="rest-overlay">
      <div className="rest-num">{left}</div>
      <div><div className="rest-lbl">Rest</div><div className="rest-sub">Next exercise in {left}s</div></div>
    </div>
  );
}
...
// Small wrapper to autoplay muted demos and hide native controls where possible.
function VideoDemo({ src, poster, style, ariaLabel }) {
  const ref = useRef(null);
  useEffect(() => {
    ...
    // Prefer autoplay muted loop when user hasn't requested reduced motion.
    if (prefersReduce) {
      el.controls = true;
      el.preload = "metadata";
    } else {
  ...
  return <video
    ...
    onError={e => { try { const c = e.target.closest && e.target.closest('div'); if (c) c.style.display = 'none'; } catch (err) {} }}
    onLoadedMetadata={e => { try { e.target.style.display = 'block'; } catch (err) {} }}
  />;
}

function Steps({ steps, small, start: startText, demoVideo }) {
  const [collapsed, setCollapsed] = useState(false);
  return (
    <div className={small ? "steps" : "fc-steps"}>
      {!small && (
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", borderBottom: "1px solid var(--border)" }}>
          <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "inherit" }}>INSTRUCTIONS</div>
          <button onClick={() => setCollapsed(c => !c)} style={{ background: "transparent", border: "none", color: "inherit", cursor: "pointer", fontWeight: 700 }}>
            {collapsed ? "Show" : "Hide"}
          </button>
        </div>
      )}
      {small && (steps?.length > 0 || startText) && (
        <div style={{ display: "none", fontSize:9, letterSpacing:"0.22em", textTransform:"uppercase",
          color:"var(--text-dimmer)", fontWeight:600, paddingBottom:8,
          borderBottom:"1px solid var(--border)", marginBottom:0, paddingLeft:0}}>
          How to perform
        </div>
      )}

      {/* Demo video for small (list) view */}
      {(!collapsed) && small && demoVideo && (
        <div style={{ padding: "10px 16px 8px", textAlign: "center", borderBottom: steps?.length ? "1px solid var(--border)" : "none", marginBottom: 6 }}>
          <VideoDemo
            src={demoVideo}
            style={{ width: "100%", borderRadius: 8, display: "block" }}
            ariaLabel="Exercise demo"
          />
        </div>
      )}
      {/* START row — no badge, matches Guided \"START\" block */}
      {(!collapsed) && startText && (small ? (
        <div className="step">
          <div className="step-right">
            <div className="step-focus" style={{marginBottom:4}}>Start</div>
            <div className="step-cue">{startText}</div>
          </div>
        </div>
      ) : (
        <div className="fc-step">
          <div className="fc-right">
            <div className="fc-pfocus">Start</div>
            <div className="fc-pcue">{startText}</div>
          </div>
        </div>
      ))}

      {/* Steps: numeric badges 1,2,3… with focus + cue, matching Guided card */}
      {steps?.map((s, i) => small ? (
        <div className="step" key={i}>
          <div className="step-badge">
            <div className="step-badge-inner">
              <span className="step-pos">{i + 1}</span>
            </div>
          </div>
          <div className="step-right">
            <div className="step-focus">{s.focus || (i === 0 ? "Setup" : "Move")}</div>
            <div className="step-cue">{s.cue}</div>
          </div>
        </div>
      ) : (
        <div className="fc-step" key={i}>
          <div className="fc-badge">
            <span className="fc-pos">{i + 1}</span>
          </div>
          <div className="fc-right">
            <div className="fc-pfocus">{s.focus || (i === 0 ? "Setup" : "Move")}</div>
            <div className="fc-pcue">{s.cue}</div>
          </div>
        </div>
      ))}
    </div>
...
function ExRow({ ex, done, onToggle, open, onExpand, skipped, onSkip, faved, onFav, note, onNote, nextName, hideTimer = false, hideDone = false, exerciseDurationSeconds }) {
  ...
  return (
    <>
      <div ref={rowRef} className={`er ${open ? "open" : ""} ${skipped ? "skipped" : ""}`} onClick={onExpand} style={skipped ? { opacity: 0.32 } : {}}>
        {!hideDone && <div className={`chk ${done && !skipped ? "on" : ""}`} onClick={e => { e.stopPropagation(); if (!skipped) onToggle(); }}>
          <svg width="9" height="7" viewBox="0 0 9 7" fill="none">
            <path d="M1 3.5L3.2 5.5L8 1" stroke="#13181B" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </div>}
        <div className="ei">
          <div className={`en ${done && !skipped ? "done" : ""} ${skipped ? "skipped" : ""}`}>{ex.name}</div>
          <div className="es">{ex.sub}</div>
          {!skipped && !hideDone && (() => {
            ...
            const displayText = useOverride
              ...
              : ex.reps;
            return displayText ? <div className="ereps">{displayText}</div> : null;
          })()}
        </div>
        <div className="row-actions" onClick={e => e.stopPropagation()}>
          {!skipped && <button className={`ra-btn ${faved ? "fav-on" : ""}`} onClick={onFav}>★</button>}
          <button className={`ra-btn ${skipped ? "skip-on" : ""}`} onClick={onSkip}>{skipped ? (
        ...
        <div className={`chev ${open ? "op" : ""}`} style={{ marginLeft: 4 }}>▾</div>
      </div>
      {open && (
        <div className="panel" style={skipped ? { opacity: 0.45 } : {}}>
          <Steps
            steps={ex.steps}
            small
          ...
          {!skipped && (
            <>
              {(ex.type === "time" || ex.type === "flow") && !hideTimer
                ? <Timer key={ex.id} seconds={exerciseDurationSeconds != null ? exerciseDurationSeconds : ex.seconds} onDone={markDoneAndCollapse} nextName={nextName} />
                : ex.reps && !hideDone && ex.id !== 5 && ex.id !== 702
                  ? <div style={{marginTop:20, paddingTop:16, borderTop:"1px solid var(--border)"}}>
                      <div style={{fontSize:9, letterSpacing:"0.22em", textTransform:"uppercase", color:"var(--accent-secondary)", fontWeight:700, marginBottom:8}}>Target</div>
                      <div style={{fontSize:16, fontWeight:700, letterSpacing:"0.04em", color:"var(--text-white)", lineHeight:1.3, marginBottom:16}}>
                        {ex.reps}
                      </div>
                      <button className="mark-btn" onClick={e => { e.stopPropagation(); markDoneAndCollapse(); }}>Mark Done</button>
                    </div>
                  : ex.reps && (ex.id === 5 || ex.id === 702) && !hideDone
                  ? <button className="mark-btn" onClick={e => { e.stopPropagation(); markDoneAndCollapse(); }} style={{ marginTop:16 }}>Mark Done</button>
                  : !hideDone ? <button className="mark-btn" onClick={e => { e.stopPropagation(); markDoneAndCollapse(); }} style={{ marginTop:16 }}>Mark Done</button> : null
              }
            </>
          )}
...
function MomentumMap({ history }) {
  ...
  return (
    <div className="summary summary-momentum" style={{ marginTop: 24 }}>
      <div className="summary-title" style={{ marginBottom: 12 }}>Momentum</div>
      <div className="momentum-strip">
        {days.map((dateStr, i) => {
          ...
          const isToday = i === 6;
          return (
            <div key={i} className="momentum-cell" style={{
      ...
      <div className="momentum-labels">
        <span className="momentum-label">7 days ago</span>
        <span className="momentum-label">Today</span>
      </div>
      <div className="momentum-disclaimer">
        Activity data is stored locally on this device.
      </div>
    </div>
  );
}
...
//  TAB BAR
// ─────────────────────────────────────────────────────────────
function TabBar({ view, setView, hasActiveSession, theme, nightMode = false }) {
  const primaryColor = "var(--accent)";
  const defaultColor = "#8E9BA5";
  ...
  const SessionIcon = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="8 12 11 15 16 9"/></svg>;
  const MoodIcon = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="5"/></svg>;
  const TimerIcon = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="13" r="8"/><path d="M12 9v4l2.5 2.5"/><path d="M9 3h6"/><path d="M12 3v2"/></svg>;
  ...
  const bar = (
    <div className="tab-bar" data-theme={theme} data-night={nightMode ? "true" : "false"}>
      <div className="tab-bar-top-line" style={{position:"absolute",top:0,left:0,right:0,height:1,background:"linear-gradient(90deg,transparent,var(--glass-border) 50%,transparent)",pointerEvents:"none"}} />
      <button
        className={`tab-btn ${view==="home"?"active":""}`}
        onClick={()=>setView("home")}
        style={{ color: view==="home" ? primaryColor : defaultColor }}
        aria-label="Today"
      ...
      <button
        ...
        style={{ position:"relative", color: view==="session" ? primaryColor : defaultColor }}
        aria-label="Session"
        aria-current={view==="session"?"page":undefined}
      >
        {hasActiveSession && view!=="session" && <div className="tab-live-dot" aria-hidden/>}
        <SessionIcon/>
        Session
      </button>
      <button
        ...
        style={{ color: view==="timer" ? primaryColor : defaultColor }}
        aria-label="Timer"
      ...
      <button
        ...
        style={{ color: view==="mood" ? primaryColor : defaultColor }}
        aria-label="Mood and appearance"
      ...
      <button
        ...
        style={{ color: view==="settings" ? primaryColor : defaultColor }}
        aria-label="Settings"
...
function OverlayTimer({ seconds, onDone, autoStart = false, paused = false, onReset, fi = 0, list = [], theme = "dark", activePeriod = null, nightMode = false, large = false, compactBarGap = false }) {
  ...
  if (large) {
    ...
    return (
      <div style={{display: "flex", flexDirection: "column", alignItems: "center", marginTop}}>
        {/* Number on top — uses main theme accent on ACTIVE BEGIN */}
        ...
        {/* Progress bar: matches GET READY (249×8); compactBarGap for GUIDED locked position */}
        <div style={{width: barWidth, height: barHeight, borderRadius: barHeight, background: BAR_TRACK, overflow: "hidden", marginTop: compactBarGap ? 14 : 28}}>
          <div style={{
            height: barHeight,
  ...
  const displayColor = isTimerDark ? "#ffffff" : "#1a1048";
  return (
    <div style={{display:"flex", flexDirection:"column", gap:gap, marginTop, alignItems:"flex-start"}}>
      <div style={{display:"flex", alignItems:"baseline", gap: 6, marginBottom: marginBottom}}>
        <div style={{minWidth: 100, textAlign: "left", fontVariantNumeric: "tabular-nums", fontFamily: "'Barlow', sans-serif", fontSize: numSize, fontWeight:600, letterSpacing:"-0.04em", color:displayColor, lineHeight:1}}>{display}</div>
      </div>
      <div style={{height: barH, width: "auto", background:"transparent", borderRadius: barH, overflow:"hidden"}}>
        <div style={{height: barH, borderRadius: barH, background:BAR_FILL,
...
function GuidedOverlay({ theme, activePeriod, activeAll: activeAllProp, TOTAL, done, totalDone, sessionSecs,
  ...
  // per-exercise instruction collapse state (Guided cards)
  const [collapsedById, setCollapsedById] = useState(() => ({}));
  const toggleCollapseFor = (id) => setCollapsedById(m => ({ ...(m || {}), [id]: !m[id] }));

  // Own fi and resting state — self-contained continuous flow
  const [fi, setFi] = useState(0);
  const [phase, setPhase] = useState("intro"); // "intro" | "getready" | "launch" | "exercise" | "milestone" | "closing"
  const GETREADY_SECONDS = 15;
  const [readyLeft, setReadyLeft] = useState(GETREADY_SECONDS);
  const [paused, setPaused] = useState(false);
  const [launchCount, setLaunchCount] = useState(3); // 3, 2, 1 before exercise start
  ...
  // Display string for large countdown area (falls back to exercise duration if no running timer)
  const guidedExerciseSeconds = cur?.seconds || cur?.duration || 0;
  const guidedDefaultDisplay = guidedExerciseSeconds ? `${Math.floor(guidedExerciseSeconds / 60)}:${(guidedExerciseSeconds % 60).toString().padStart(2, "0")}` : "";
  // Guided large countdown state (per-exercise running timer)
  const [gLeft, setGLeft] = useState(null);
  const gRef = useRef(null);
  const [gRunning, setGRunning] = useState(false);

  // Start/stop guided countdown when phase changes or current exercise changes or paused toggles
  useEffect(() => {
    // clear any existing
    clearInterval(gRef.current);
    if (phase === "exercise" && cur) {
      const secs = cur.seconds || cur.duration || 0;
      setGLeft(secs);
        ...
        gRef.current = setInterval(() => {
          setGLeft(l => {
            if (l <= 1 || l == null) {
              clearInterval(gRef.current);
              setGRunning(false);
              // call handler to complete current exercise
              handleTimerDone();
              return 0;
            }
            return l - 1;
  ...
  }, [phase, fi, cur && cur.id, paused]);

  // helper display and pct for guided large timer
  const gTotal = (cur && (cur.seconds || cur.duration)) || 0;
  const gPct = gTotal > 0 && gLeft != null ? ((gTotal - gLeft) / gTotal) : (phase === "launch" ? ((3 - launchCount) / 3) : 0);
  const gDisplay = gLeft != null ? `${Math.floor(gLeft / 60)}:${(gLeft % 60).toString().padStart(2, "0")}` : guidedDefaultDisplay;

  // GUIDED phase flags
  const isExercisePhase = phase === "exercise";

  // BEGIN (exercise) uses a flat, rich theme background and higher-contrast surfaces
  const GUIDED_BG = (!isNight && isExercisePhase)
    ? (ct.bg || MAIN_APP_BG[isDark ? "dark" : "light"])
    : BG;
  ...
  // Get-ready countdown (15s) — at 0 go to 3-2-1 launch
  useEffect(() => {
  ...
  // 3-2-1 launch countdown with beeps then start exercise (primeAudio/beep on user context helps iOS)
  useEffect(() => {
    if (phase !== "launch") return;
    const t1 = setTimeout(() => { primeAudio(); setLaunchCount(2); beep(440, 0.1); }, 1000);
    const t2 = setTimeout(() => { setLaunchCount(1); beep(440, 0.1); }, 2000);
    const t3 = setTimeout(() => { setLaunchCount(0); beep(880, 0.3); setPhase("exercise"); }, 3000);
  ...
  const handleSkip = () => {
    onSkip(cur.id);
    beep(440, 0.2);
    if (fi < list.length - 1) { setFi(i => i + 1); setReadyLeft(GETREADY_SECONDS); setPhase("getready"); }
  };
  ...
  const handlePrev = () => {
    if (fi > 0) { setFi(i => i - 1); setReadyLeft(GETREADY_SECONDS); setPhase("getready"); }
  };

  // No exercises: show message and exit (avoids cur.id / cur.name access on undefined)
  if (listTotal === 0 || !cur) {
    return (
      <div data-theme={theme} data-night={isNight ? "true" : "false"} style={{
        ...
        <style>{css}</style>
        <div style={{ fontSize:18, fontWeight:600, marginBottom:12, color:W }}>No exercises in this flow</div>
        <div style={{ fontSize:14, color:S, marginBottom:24 }}>Add exercises or clear filters and try again.</div>
        <button onClick={onExit} style={{
          padding:"14px 40px", borderRadius:28, cursor:"pointer",
          background:A, border:"none", color:"#ffffff",
  ...
  return (
    <div data-theme={theme} data-night={isNight ? "true" : "false"} style={{
      position:"fixed", top:0, left:0, right:0, bottom:0, zIndex:99999,
      background: GUIDED_BG, display:"flex", flexDirection:"column",
      fontFamily:"'Barlow',sans-serif", color:W, WebkitFontSmoothing:"antialiased",
      overflow:"hidden",
    }}>
      <style>{css}</style>

      {/* Orb layer — static (hidden on BEGIN to emphasize flat background) */}
      {(!isExercisePhase || isNight) && (
        <div className="app-orbs" style={{
      ...
      {/* ── TOP BAR: Exit + Progress ring ── */}
      <div style={{
        display:"flex", alignItems:"center", justifyContent:"space-between",
        padding:"max(12px, env(safe-area-inset-top)) 20px 10px", flexShrink:0, position:"relative", zIndex:2,
      }}>
        <button onClick={onExit} style={{
          display:"flex", alignItems:"center", justifyContent:"center", width:44, height:44,
          background: "transparent",
          border: "1px solid " + (isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.15)"),
          borderRadius: 999, cursor: "pointer", color: S,
        ...
        {phase !== "intro" && phase !== "closing" && listTotal > 0 && (() => {
          const size = 46; const r = 19; const C = 2 * Math.PI * r;
          const filled = ((fi + 1) / listTotal) * C;
          const offset = C - filled;
          return (
            <div style={{ position:"relative", width:size, height:size, display:"flex", alignItems:"center", justifyContent:"center" }}>
              <svg width={size} height={size} style={{ transform:"rotate(-90deg)" }}>
                <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={isDark ? "rgba(255,255,255,0.12)" : "rgba(0,0,0,0.08)"} strokeWidth="3" />
                <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={A} strokeWidth="3" strokeLinecap="round"
                  strokeDasharray={C} strokeDashoffset={offset}
                  style={{ transition:"stroke-dashoffset 0.4s ease-out" }} />
              </svg>
              <span style={{ position:"absolute", fontSize: 11, fontWeight: 600, color: S, fontFamily: "'Barlow', sans-serif" }}>{fi + 1}/{listTotal}</span>
            </div>
          );
        })()}
      </div>

      {/* ── MILESTONE overlay (brief) ── */}
      {phase === "milestone" && (
        <div style={{
      ...
      {phase === "closing" && (
        <div style={{
          ...
          <div style={{fontSize:24, fontWeight:600, letterSpacing:"0.20em", textTransform:"uppercase", lineHeight:1.4, color:W, marginBottom:16, maxWidth:320,
            textShadow: NAME_SHAD}}>
            GUIDED COMPLETE
          </div>
          <div style={{fontSize:15, color:S, marginBottom:24, maxWidth:300, lineHeight:1.6}}>
            Take a few moments to breathe or rest.
          </div>
          <div style={{fontSize:12, color:D, marginBottom:40, maxWidth:280, lineHeight:1.6}}>
      ...
      {/* ── INTRO SCREEN: mockup — GUIDED SESSION, title, subtitle, two placeholder rects, GET READY at bottom ── */}
      {phase === "intro" && (
        <div style={{
          flex:1, display:"flex", flexDirection:"column",
          alignItems:"center", justifyContent:"space-between",
          padding:"20px 24px 20px", textAlign:"center", position:"relative", zIndex:2, minHeight:0,
        }}>
          <div style={{flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center"}}>
            <div style={{letterSpacing:"0.3em", textTransform:"uppercase", color:S, marginBottom:24}}>GUIDED SESSION</div>
            <div style={{fontWeight:700, fontSize: 34, letterSpacing:"-0.03em", lineHeight:1.02, color:W, marginBottom:16, textShadow: NAME_SHAD}}>
              {trackLabel}
            </div>
            <div style={{fontSize: 20, color: S, marginBottom: 24, opacity: 0.9}}>{filteredCount} exercises • {trackDuration}</div>
          </div>
          <div style={{display:"flex", flexDirection:"column", alignItems:"center", paddingBottom:8}}>
            <div style={{fontSize: 12, color: S, opacity: 0.85, maxWidth: 320, lineHeight: 1.4, marginBottom: 28, textAlign: "center"}}>No equipment? You can modify — use a towel instead of a strap, a book instead of a block, or skip props and focus on range of motion.</div>
            <button onClick={() => { primeAudio(); beep(880, 0.3); setReadyLeft(GETREADY_SECONDS); setPhase("getready"); }} style={{
            ...
            }}>REST & GET READY</button>
            <button onClick={onExit} style={{
              marginTop:14, background:"none", border:"none", cursor:"pointer",
              fontSize:11, letterSpacing:"0.16em", textTransform:"uppercase", color:D, fontFamily:"'Barlow',sans-serif",
            }}>Cancel</button>
          </div>
        </div>
      )}

      {/* ── GET READY / REST / EXERCISE — top: copy (scrollable); bottom half: large timer in liquid glass ── */}
      {phase !== "intro" && phase !== "closing" && phase !== "milestone" && (
        <div style={{flex:1, display:"flex", flexDirection:"column", position:"relative", zIndex:2, minHeight:0, overflow:"hidden"}}>
          {/* Top: instructional copy — flex column so timer/launch can sit at bottom (marginTop: auto) */}
          <div style={{flex: "1 1 58%", minHeight:0, overflowY:"auto", overflowX:"hidden", padding:"6px 16px 12px", WebkitOverflowScrolling:"touch", display:"flex", flexDirection:"column"}}>
          {/* Status label: keep only for non-guided-timer phases; GET READY / GET SET / BEGIN now live in the pill */}
          <div style={{ fontSize: 13, letterSpacing: "0.28em", textTransform: "uppercase", color: S, marginBottom: 8, flexShrink: 0, textAlign: "left" }}>
            {phase !== "getready" && phase !== "launch" && phase !== "exercise" ? (phase === "milestone" ? "MILESTONE" : "") : ""}
          </div>

          {/* Primary title: large, bold — white, centered (stays locked across phases) */}
          {/* Phase label and large countdown at top */}
          {(phase === "getready" || phase === "launch" || phase === "exercise") && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 8 }}>
              <div style={{
                ...
                {phase === "getready" ? "REST & GET READY" : phase === "launch" ? "GET SET" : "BEGIN"}
              </div>
            </div>
          )}

          {/* Primary title: large, bold — white, centered (stays locked across phases) */}
          <div style={{display:"flex",flexDirection:"column",alignItems:(phase === "getready" || phase === "launch" || phase === "exercise") ? "center" : "flex-start"}}>
            <div
              ref={titleRef}
              style={{
              fontSize: (phase === "getready" || phase === "launch" || phase === "exercise") ? 34 : 30,
              fontWeight: 700, letterSpacing: "0.02em", lineHeight: 1.08, textTransform: "uppercase",
              color: W,
              marginBottom: 8, flexShrink: 0, fontFamily: "'Barlow', sans-serif",
              textShadow: NAME_SHAD,
              textAlign: (phase === "getready" || phase === "launch" || phase === "exercise") ? "center" : "left",
            }}>
              {cur?.name}
            </div>
          </div>

          {/* Countdown + progress bar: locked position for getready, launch, and exercise (placed under title) */}
          {(phase === "getready" || phase === "launch" || phase === "exercise") && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginTop: 6, marginBottom: 12, flexShrink: 0 }}>
              {phase === "getready" && (
                <>
                  <div style={{ minWidth: 140, textAlign: "center", fontVariantNumeric: "tabular-nums", fontFamily: "'Barlow', sans-serif", fontSize: 64, fontWeight: 700, letterSpacing: "-0.02em", color: isDark ? "rgba(255,255,255,0.5)" : W, lineHeight: 1 }}>
                    {`0:${readyLeft.toString().padStart(2, "0")}`}
                  </div>
                  <div style={{ width: 240, height: 10, borderRadius: 10, background: isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.08)", overflow: "hidden", marginTop: 8 }}>
                    <div style={{ height: 10, borderRadius: 10, background: `linear-gradient(90deg,${A},${ct.accentDim || A + "99"})`, width: `${((GETREADY_SECONDS - readyLeft) / GETREADY_SECONDS) * 100}%`, transition: "width 1s linear" }} />
                  </div>
                </>
              )}
              {phase === "launch" && (
                <>
                  <div style={{ minWidth: 140, textAlign: "center", fontVariantNumeric: "tabular-nums", fontFamily: "'Barlow', sans-serif", fontSize: 60, fontWeight: 700, letterSpacing: "-0.02em", color: W, lineHeight: 1 }}>
                    {launchCount > 0 ? launchCount : ""}
                  </div>
                  <div style={{ width: 240, height: 10, borderRadius: 10, background: isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.08)", overflow: "hidden", marginTop: 8 }}>
                    <div style={{ height: 10, borderRadius: 10, background: `linear-gradient(90deg, ${A}, ${A})`, width: `${((3 - launchCount) / 3) * 100}%` }} />
                  </div>
                </>
              )}
              {phase === "exercise" && cur && (
                <>
                  <div style={{ minWidth: 140, textAlign: "center", fontVariantNumeric: "tabular-nums", fontFamily: "'Barlow', sans-serif", fontSize: 60, fontWeight: 700, letterSpacing: "-0.02em", color: W, lineHeight: 1 }}>{gDisplay}</div>
                  <div style={{ width: 240, height: 10, borderRadius: 10, background: isDark ? "rgba(255,255,255,0.10)" : "rgba(0,0,0,0.08)", overflow: "hidden", marginTop: 8 }}>
                    <div style={{ height: 10, borderRadius: 10, background: `linear-gradient(90deg, ${A}, ${A})`, width: `${gPct * 100}%` }} />
                  </div>
                </>
              )}
            </div>
          ...
          {(() => {
            ...
            return (
              <div style={{
                borderRadius: 20,
                background: isExercisePhase && !isNight
                  ? (isDark ? "rgba(6,14,32,0.96)" : "rgba(252,252,255,0.98)")
                  : (isDark ? "rgba(255,255,255,0.06)" : "rgba(255,255,255,0.45)"),
                backdropFilter: isExercisePhase && !isNight ? "none" : "blur(24px) saturate(1.4)",
                WebkitBackdropFilter: isExercisePhase && !isNight ? "none" : "blur(24px) saturate(1.4)",
                border: isExercisePhase && !isNight
                  ? (isDark ? "1px solid rgba(255,255,255,0.22)" : "1px solid rgba(10,18,36,0.08)")
                  : `1px solid ${isNight ? "rgba(255,59,48,0.12)" : (isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.08)")}`,
                boxShadow: isExercisePhase && !isNight
                  ? (isDark ? "0 8px 26px rgba(0,0,0,0.55)" : "0 10px 32px rgba(12,24,48,0.26)")
                  : (isDark ? "0 4px 24px rgba(0,0,0,0.15)" : "0 4px 20px rgba(0,0,0,0.06)"),
                overflow: "hidden", flexShrink: 0, marginBottom: 16,
              }}>
                {/* Card header with collapse control */}
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 16px", borderBottom: "1px solid rgba(0,0,0,0)" }}>
                  <div style={{ fontSize: 13, fontWeight: 700, letterSpacing: "0.06em", color: W }}>Instructions</div>
                  <button
                    onClick={() => toggleCollapseFor(ex.id)}
                    style={{ background: "transparent", border: "none", color: W, cursor: "pointer", fontWeight: 700, fontSize: 13 }}
                    aria-expanded={!collapsedById[ex.id]}
                    aria-controls={`instr-${ex.id}`}
                  >
                    {collapsedById[ex.id] ? "Show" : "Hide"}
                  </button>
                </div>
                ...
                {!collapsedById[ex.id] && (
                  <>
                    {ex.demoVideo && (
                      <div style={{ padding: "10px 16px 8px", borderBottom: ex.steps?.length ? (isDark ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.06)") : "none", textAlign: "center" }}>
                        <VideoDemo
                          src={ex.demoVideo}
                          style={{ width: "100%", borderRadius: 10, display: "block" }}
                          ariaLabel={ex && ex.name ? `Demo: ${ex.name}` : "Exercise demo"}
                        />
                      </div>
                    )}
                    {ex.start && (
                      <div id={`instr-${ex.id}`} style={{ padding: "10px 16px 8px", borderBottom: ex.steps?.length ? (isDark ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.06)") : "none", textAlign: "left" }}>
                        <div style={{ fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: A, marginBottom: 4 }}>START</div>
                        <div style={{ fontSize: 15, color: W, lineHeight: 1.4, fontWeight: 400, letterSpacing: "-0.01em" }}>{ex.start}</div>
                      </div>
                    )}
                    {ex.steps?.map((step, idx) => (
                      <div
                        key={idx}
                        style={{
                          padding: idx === 0 ? "14px 16px 12px" : "12px 16px 14px",
                          borderBottom: (idx < ex.steps.length - 1) || (ex.type === "reps" && ex.reps)
                            ? (isDark ? "1px solid rgba(255,255,255,0.08)" : "1px solid rgba(0,0,0,0.06)")
                            : "none",
                          textAlign: "left",
                        }}
                      >
                        <div style={{ fontSize: 11, letterSpacing: "0.22em", textTransform: "uppercase", color: A, marginBottom: 8 }}>
                          {step.focus || (idx === 0 ? "SETUP" : "MOVE")}
                        </div>
                        <div style={{
                          fontSize: 15,
                          color: W,
      ...
      {/* ── LIQUID GLASS PILL BAR (Exit, Pause, Next, Skip Rest) — hidden on intro ── */}
      {phase !== "closing" && phase !== "milestone" && phase !== "intro" && (
      <div style={{
        margin:"0 16px", marginBottom:`max(28px, env(safe-area-inset-bottom, 28px))`,
        flexShrink:0, position:"relative", zIndex:2,
        display:"flex", alignItems:"stretch", height:56, gap: phase === "exercise" ? 12 : 0,
        ...(phase === "exercise" ? {} : { borderRadius: 24, overflow: "hidden", background: BAR_BG, backdropFilter: isDark ? "blur(40px) saturate(2)" : "blur(28px) saturate(1.4)", WebkitBackdropFilter: isDark ? "blur(40px) saturate(2)" : "blur(28px) saturate(1.4)", border: BAR_BDR, boxShadow: BAR_SHAD }),
      }}>
        {/* EXERCISE: two individual pills — Pause + Next/Done */}
        {phase === "exercise" && cur && (
          <>
            <button onClick={() => setPaused(p => !p)} style={{
              flex: 1, minWidth: 0, borderRadius: 999,
              ...
              transition: "all 0.2s", boxShadow: isDark ? "0 4px 20px rgba(0,0,0,0.2)" : "0 4px 16px rgba(0,0,0,0.08)",
            ...
            <button onClick={handleNext} style={{
              ...
              fontFamily: "'Barlow', sans-serif", fontSize: 11, fontWeight: 700,
              letterSpacing: "0.18em", textTransform: "uppercase", cursor: "pointer",
              boxShadow: `0 0 16px ${ct.accentGlow}`,
            }}>{fi === list.length - 1 ? "DONE" : "NEXT"}</button>
          </>
        )}

        {/* GET READY: Start Now — full width (triggers 3-2-1 then exercise; primeAudio helps iOS unlock beeps) */}
        {phase === "getready" && (
          <button onClick={() => { clearInterval(readyRef.current); primeAudio(); setLaunchCount(3); setPhase("launch"); }} style={{
            flex: 1, width: "100%", minWidth: 0, background: A, border: "none",
            color: "#ffffff", fontFamily: "'Barlow', sans-serif", fontSize: 12, fontWeight: 700,
            letterSpacing: "0.2em", textTransform: "uppercase", cursor: "pointer",
            boxShadow: `0 0 20px ${ct.accentGlow}`,
          }}>START NOW</button>
        )}

        {/* LAUNCH: no button — 3-2-1 runs automatically */}
        {phase === "launch" && (
          <div style={{ flex: 1, width: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: S, fontSize: 12, letterSpacing: "0.12em" }}>STARTING...</div>
        )}
      </div>
      )}
    </div>
...
function TimerView({ theme, view, setView, hasActiveSession, css, nightMode = false, activePeriod = null }) {
  ...
  // Breathe intensity (4s inhale, 7s hold, 8s exhale): one formula per phase, no div-by-zero
  const phaseDur = Math.max(1, curBreathPhase?.dur ?? 1);
  const progress = 1 - bTime / phaseDur; // 0 at phase start (e.g. 4), 1 at phase end (0)
  let breatheIntensity = 1;
  if (isInhalePhase) {
    const lastSecondProgress = 1 - 1 / phaseDur; // e.g. 0.75 for 4s — full intensity once we're at 1
    breatheIntensity = progress >= lastSecondProgress ? 1 : progress / lastSecondProgress; // 0 at 4 (off), quickly grow to 1 by "1"
  } else if (isHoldPhase) {
    breatheIntensity = 1;
  } else if (isExhalePhase) {
    const isLastRound = bCycle >= breathCycles;
    breatheIntensity = isLastRound && bTime <= 1 ? 0 : 0.25 + 0.75 * (1 - progress); // 1 at 8 → 0.25; only in 4th round at 1: fade out completely
  }
  const orbOpacity = breatheIntensity;
  ...
  // compute ring progress (0..1) per phase for SVG stroke
  let breatheProgress = 1;
  if (isInhalePhase) {
    breatheProgress = (phaseDur - bTime) / phaseDur; // 0 -> 1 over inhale
  } else if (isHoldPhase) {
    breatheProgress = 1;
  } else if (isExhalePhase) {
    breatheProgress = bTime / phaseDur; // 1 -> 0 over exhale
  }
  const r = 120;
  ...
  function bStart() {
    ...
    // ensure scale transition runs on first inhale: set running first, then expand after a brief tick
    setBExpand(false);
    setBRunning(true);
    // defer setting expand so CSS transition animates from small -> full
    setTimeout(() => {
  ...
  const timerNumMedium = { fontFamily: "'Barlow', sans-serif", fontWeight: 600, fontSize: 22, color: "var(--text-white)", lineHeight: 1, minWidth: 40, textAlign: "center", fontVariantNumeric: "tabular-nums" };
  const timerNumSmall = { fontFamily: "'Barlow', sans-serif", fontWeight: 500, fontSize: 13, color: "var(--text-dimmer)", lineHeight: 1.2 };
  const BTN       = { padding:"12px 32px", borderRadius:28, cursor:"pointer", fontFamily:"'Barlow',sans-serif", fontSize:"11px", fontWeight:700, letterSpacing:"0.16em", textTransform:"uppercase", transition:"all 0.22s" };
  const BTN_GHOST = { padding:"12px 20px", borderRadius:28, cursor:"pointer", fontFamily:"'Barlow',sans-serif", fontSize:"11px", fontWeight:600, letterSpacing:"0.12em", textTransform:"uppercase", transition:"all 0.22s" };
  // Lock INTERVAL + BREATHE layout: same ring slot, same mid slot, same button row (BREATHE as reference — no extra ring offset)
  // Circle as large as possible within column; static slot so INTERVAL/BREATHE don’t shift
  const ringSize = 360;
  ...
  const breatheInset = (ringSize - 306) / 2; // 27 when ringSize 360 — same ring as INTERVAL
  const TIMER_MID_MIN_IDLE = 156; // min height for content
  const TIMER_MID_IDLE_HEIGHT = 176; // fixed height when idle so INTERVAL and BREATHE START align
  const TIMER_PHASE_BAND = 52;    // matches BREATHE phase label block height so Pause/Reset align when running
  // Toggle: spacer above so INTERVAL/BREATHE sit halfway between top edge and top of circle
  const TIMER_TOGGLE_SPACER_TOP = 12;
  const TIMER_TOGGLE_WRAP = { flexShrink:0, display:"flex", gap:10, marginBottom:14, padding:6, minHeight:48, alignItems:"center", boxSizing:"border-box" };
  ...
  // Idle: START halfway between Work/Rest/Rounds (or patterns) and tab bar — balanced flex + no pull-up
  const TIMER_BTN_SPACER_ABOVE_IDLE = { flex:0.35, minHeight:0, width:"100%" };
  const TIMER_BTN_SPACER_BELOW_IDLE = { flex:1.65, minHeight:0, width:"100%" };
  const TIMER_BTN_ROW = { flexShrink:0, display:"flex", gap:8, width:"100%", justifyContent:"center", boxSizing:"border-box", paddingTop:6, paddingBottom:6 };
  ...
  .timer-view-body .timer-glass-toggle-btn.active {
    background: var(--accent); color: var(--accent-btn-text, #fff);
  }
  ...
  /* Single soft glow behind the ring (SVG will be used for glow and crisp ring) */
  .timer-view-body .timer-breathe-glow {
    ...
    filter: blur(6px);
    transition: opacity var(--breathe-fade, 0.85s) ease, filter 1.2s ease;
  }
  ...
  return (
      <div className="app" data-theme={theme} data-night={nightMode ? "true" : "false"} style={{minHeight:"100dvh",display:"flex",flexDirection:"column",minWidth:"100%",boxSizing:"border-box"}}>
      <style>{css}</style>
      <style>{timerFlatCss}</style>
      <div className="app-orbs" style={{background:"var(--orb1), var(--orb2), var(--orb3)", transition:"background 0.4s ease"}} />
      {/* Anchor from top like first paint — no vertical centering so circle/toggle don’t jump when mode/state changes */}
      <div className="app-body timer-view-body" style={{
        display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",
        flex:1,minHeight:0,width:"100%",
        padding:"calc(env(safe-area-inset-top, 0px) + 16px) 24px calc(84px + env(safe-area-inset-bottom, 0px))",
        boxSizing:"border-box",
      }}>
        {/* Single column: spacer → toggle (halfway top→circle) → ring → mid → spacers → buttons (halfway cycles→tab bar) */}
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%",maxWidth:360,flex:1,minHeight:0}}>
        <div style={{flexShrink:0, height:TIMER_TOGGLE_SPACER_TOP, width:"100%"}} aria-hidden />
        <div className="timer-mode-outer-pill timer-glass-wrap" style={TIMER_TOGGLE_WRAP}>
          {["interval","breathe"].map(m => (
            <button key={m} type="button" onClick={()=>switchMode(m)} className={"timer-glass-toggle-btn" + (mode===m ? " active" : "")}>{m === "interval" ? "Interval" : "Breathe"}</button>
          ))}
        </div>

        {/* ── INTERVAL MODE ── circle static above; Work/Rest/Rounds then START/PAUSE/RESET below */}
        {mode === "interval" && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%",flex:1,minHeight:0}}>
          <div className="timer-glass-orb timer-ring-slot" style={TIMER_RING_WRAP}>
            <svg width={ringSize} height={ringSize} viewBox="0 0 320 320" className="timer-interval-ring" style={{transform:"rotate(-90deg)",display:"block",margin:"0 auto"}}>
              <circle cx="160" cy="160" r="140" fill="none" stroke="var(--border)" strokeWidth="2.5"/>
              {iPhase !== "idle" && iPhase !== "done" && (
            ...
            {/* Overlay: big seconds centered in circle; rounds row in bottom 1/4 (80px) */}
            <div style={{position:"absolute",left:0,top:0,width:ringSize,height:ringSize,pointerEvents:"none"}}>
              {/* Large countdown — centered in full circle */}
              <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                {iPhase === "idle" && (
                  <div style={intervalDigitStyle}>{formatSecondsOnly(work)}</div>
                )}
                {(iPhase === "work" || iPhase === "rest") && (
                  <div style={{...intervalDigitStyle, transition:"color 0.4s"}}>{formatSecondsOnly(iTime)}</div>
                )}
                {iPhase === "done" && (
                  <div style={{fontSize:"48px",fontWeight:700,color:"var(--accent)",fontFamily:"'Barlow',sans-serif",lineHeight:1}}>✓</div>
                )}
              </div>
              {/* Rounds counter — bottom quarter band, shifted up 1/4 of band (20px) toward countdown */}
              {(iPhase === "idle" || iPhase === "work" || iPhase === "rest") && (
                <div style={{
                  position:"absolute",left:0,right:0,bottom:20,height:80,
                  display:"flex",alignItems:"center",justifyContent:"center",gap:6,
                }}>
                  <span style={{...timerNumSmall,fontSize:12,letterSpacing:"0.06em"}}>
                    {(iPhase === "idle" ? 1 : iRound)} of {rounds}
                  </span>
                </div>
              )}
            </div>
          ...
          {/* Mid zone: Work/Rest/Rounds below circle when idle; fixed height so START matches BREATHE */}
          <div style={{flexShrink:0,height:iPhase==="idle"?TIMER_MID_IDLE_HEIGHT:undefined,minHeight:iPhase==="idle"?TIMER_MID_IDLE_HEIGHT:0,width:"100%",marginTop:12,marginBottom:12,boxSizing:"border-box"}}>
          {iPhase === "idle" && (
            <div className="timer-glass-panel timer-interval-rows" style={{marginBottom:0,width:"100%",boxSizing:"border-box",overflow:"hidden",borderRadius:16}}>
              {[
              ...
              ].map(({label,val,set,min,max,step,isRound}) => (
                <div key={label} className="timer-interval-row">
                  <span className="timer-interval-row-label">{label}</span>
                  <div className="timer-interval-row-controls">
                    <button type="button" aria-label={`${label} decrease`} onClick={()=>set(v=>Math.max(min,v-step))} className="timer-glass-stepper timer-interval-stepper" style={{color:"var(--text-secondary)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",border:"none"}}>−</button>
                    <div className="timer-interval-row-value" style={{display:"flex",alignItems:"baseline",justifyContent:"center",gap:2}}>
                      <span style={{...timerNumMedium,fontSize:20}}>{val}</span>
                      {!isRound && <span style={{fontSize:12,color:"var(--text-dimmer)",fontFamily:"'Barlow',sans-serif"}}>s</span>}
                    </div>
                    <button type="button" aria-label={`${label} increase`} onClick={()=>set(v=>Math.min(max,v+step))} className="timer-glass-stepper timer-interval-stepper" style={{color:"var(--text-secondary)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",border:"none"}}>+</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Spacer when running/done so Pause/Again align with BREATHE Pause/Again (same vertical slot) */}
          {(iPhase === "work" || iPhase === "rest" || iPhase === "done") && (
            <div style={{flexShrink:0,width:"100%",minHeight:TIMER_PHASE_BAND,boxSizing:"border-box"}} aria-hidden />
          )}

          <div style={iPhase==="idle"?TIMER_BTN_SPACER_ABOVE_IDLE:TIMER_BTN_SPACER_ABOVE} aria-hidden />
          <div style={iPhase==="idle"?TIMER_BTN_ROW_IDLE:TIMER_BTN_ROW}>
            {iPhase === "idle"   && <button type="button" onClick={()=>{primeAudio();iStart();}} className="timer-glass-btn-pri" style={BTN}>Start</button>}
            {(iPhase==="work"||iPhase==="rest") && <>
              <button type="button" onClick={()=>setIRunning(r=>!r)} className="timer-glass-btn-pri" style={{...BTN,padding:"12px 28px"}}>{iRunning?"Pause":"Resume"}</button>
              <button type="button" onClick={iReset} className="timer-glass-btn-ghost" style={BTN_GHOST}>Reset</button>
            </>}
            {iPhase === "done"   && <button type="button" onClick={iReset} className="timer-glass-btn-pri" style={BTN}>Again</button>}
          </div>
          <div style={iPhase==="idle"?TIMER_BTN_SPACER_BELOW_IDLE:TIMER_BTN_SPACER_BELOW} aria-hidden />
        </div>)}

        {/* ── BREATHE MODE ── circle static above; patterns + Cycles then START/PAUSE/RESET below */}
        {mode === "breathe" && (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",width:"100%",flex:1,minHeight:0}}>
          <div className="timer-glass-orb timer-breathe-ring-wrap timer-ring-slot" style={TIMER_RING_WRAP}>
            <svg width={ringSize} height={ringSize} viewBox="0 0 320 320" style={{position:"absolute",top:0,left:0}} className="timer-breathe-guide-ring">
              <circle cx="160" cy="160" r="140" fill="none" stroke="var(--border)" strokeWidth="1.5"/>
            </svg>
            {bRunning && !nightMode && (
                <div className={isHoldPhase ? "timer-breathe-hold" : ""} style={{
                ...
                // precise scale computed from rendered radii and stroke widths so inner stroke edge meets outer stroke edge
                // outer rendered radius = (ringSize / 320) * 140; outer stroke half = (ringSize / 320) * (1.5/2)
                // inner rendered radius = 120; inner stroke half = 4/2
                // computed scale ≈ 1.298
                transform: bRunning && bExpand ? "scale(1.298)" : "scale(0.45)",
                transformOrigin:"center center",
                transition: bRunning ? `transform ${curBreathPhase?.dur || 4}s cubic-bezier(0.4,0,0.2,1)` : "transform 0.6s ease",
              }}>
                {/* Glow behind the ring (SVG slightly larger to avoid blur clipping) */}
                <svg className="timer-breathe-glow" width="320" height="320" viewBox="0 0 320 320" style={{position:"absolute", left:-7, top:-7, opacity: Math.max(orbOpacity ?? 0, 0.18), ['--breathe-fade']: `${breatheFadeDuration}s`}}>
                  <defs>
                  ...
                  <circle cx="160" cy="160" r="140" fill="url(#breatheGlow)" />
                </svg>

                {/* Animated inner ring using the existing inner circle (fills to match outer) */}
                <svg className="timer-breathe-ring" width="306" height="306" viewBox="0 0 306 306" style={{position:"absolute", left:0, top:0, pointerEvents:"none"}}>
                  <circle
                    cx="153"
                    cy="153"
                    r="120"
            ...
            {/* Same layout as INTERVAL: countdown centered; cycle line in bottom band */}
            <div style={{position:"absolute",left:0,top:0,width:ringSize,height:ringSize,pointerEvents:"none"}}>
              {!bRunning && !bDone && (
                <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <div style={{fontSize:"20px",fontWeight:600,letterSpacing:"0.12em",textTransform:"uppercase",color:"var(--text-dimmer)",fontFamily:"'Barlow',sans-serif"}}>Ready</div>
                </div>
              )}
              {bRunning && (
                <>
                  {/* Countdown in circle — white, fade in/out with timer */}
                  <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <div style={{...intervalDigitStyle, fontSize:72, minWidth:72, color: isDark ? "var(--app-white)" : "var(--app-black)", opacity:countdownLabelOpacity, transition: "none"}}>{formatSecondsOnly(bTime)}</div>
                  </div>
                </>
              )}
              {bDone && (
                <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <div style={{fontSize:"42px",fontWeight:700,color:"var(--accent)",fontFamily:"'Barlow',sans-serif",lineHeight:1}}>✓</div>
                </div>
              )}
            </div>
          </div>
          {/* Phase label — white (INHALE/HOLD/EXHALE), fade in/out with timer */}
          {bRunning && (
            <div className="timer-glass-phase" style={{flexShrink:0,width:"100%",textAlign:"center",padding:"9px 20px 10px",marginTop:-5,boxSizing:"border-box",minHeight:TIMER_PHASE_BAND,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{
                fontSize:26,
                fontWeight:700,
                letterSpacing:"0.22em",
                textTransform:"uppercase",
                color: isDark ? "var(--app-white)" : "var(--app-black)",
                opacity:countdownLabelOpacity,
                fontFamily:"'Barlow',sans-serif",
                transition:`opacity ${breatheOpacityTransition}`,
                lineHeight:1.2,
              }}>{curBreathPhase.label}</div>
            </div>
          )}
          {/* Cycles pill shown during active run, placed below the phase label and above controls */}
          {bRunning && (
            <div style={{display:"flex",width:"100%",justifyContent:"center",marginTop:6,marginBottom:6}}>
              <div style={{display:"inline-flex",alignItems:"center",justifyContent:"center",padding:"6px 12px",borderRadius:12,background:"var(--glass-bg)",border:"1px solid var(--glass-border)",boxSizing:"border-box"}}>
                <span style={{...timerNumSmall,fontSize:12,letterSpacing:"0.06em",color:"var(--text-secondary)"}}>{bCycle} of {breathCycles}</span>
              </div>
            </div>
          )}
          {/* Spacer when done so Again aligns with INTERVAL Again */}
          {bDone && (
            <div style={{flexShrink:0,width:"100%",minHeight:TIMER_PHASE_BAND,boxSizing:"border-box"}} aria-hidden />
          )}
          {/* Options — 4-7-8 / Box / Physio Sigh + Cycles below circle when idle; same fixed height as INTERVAL so START aligns */}
          <div style={{flexShrink:0,height:(!bRunning && !bDone)?TIMER_MID_IDLE_HEIGHT:undefined,minHeight:(!bRunning && !bDone)?TIMER_MID_IDLE_HEIGHT:0,width:"100%",marginTop:12,marginBottom:12,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"flex-start",boxSizing:"border-box"}}>
          {!bRunning && !bDone && (
            <>
              <div className="timer-pattern-row" style={{width:"100%",maxWidth:360}}>
                {Object.keys(BREATH_PATTERNS).map(p => (
                  <button key={p} type="button" onClick={()=>setBreathPattern(p)} className={"timer-glass-pattern-btn" + (breathPattern===p ? " active" : "")} style={{fontFamily:"'Barlow',sans-serif",fontSize:"13px",fontWeight:600,letterSpacing:"0.14em",textTransform:"uppercase",padding:"9px 20px",cursor:"pointer",color: breathPattern===p ? "var(--accent)" : "var(--text-secondary)"}}>{p === "4-7-8" ? "4-7-8" : p === "Box" ? "Box" : "Physio Sigh"}</button>
                ))}
              </div>
              {/* cycles pill removed from idle; shown during active run to avoid overlap */}
              <div style={{width:"100%",maxWidth:360,marginTop:6,display:"flex",alignItems:"center",justifyContent:"center"}}>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <div style={{fontSize:11,letterSpacing:"0.14em",textTransform:"uppercase",color:"var(--text-dimmer)",fontFamily:"'Barlow',sans-serif",marginRight:8}}>Cycles</div>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <button type="button" aria-label="Cycles decrease" onClick={()=>setBreathCycles(v=>Math.max(1,v-1))} className="timer-glass-stepper timer-interval-stepper" style={{color:"var(--text-secondary)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",border:"none"}}>−</button>
                    <div style={{minWidth:34,textAlign:"center",fontVariantNumeric:"tabular-nums"}}><span style={{...timerNumMedium,fontSize:18}}>{breathCycles}</span></div>
                    <button type="button" aria-label="Cycles increase" onClick={()=>setBreathCycles(v=>Math.min(12,v+1))} className="timer-glass-stepper timer-interval-stepper" style={{color:"var(--text-secondary)",fontSize:18,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",border:"none"}}>+</button>
                  </div>
                </div>
              </div>
            </>
          ...
          <div style={(!bRunning && !bDone) ? TIMER_BTN_SPACER_ABOVE_IDLE : TIMER_BTN_SPACER_ABOVE} aria-hidden />
          <div style={(!bRunning && !bDone) ? TIMER_BTN_ROW_IDLE : TIMER_BTN_ROW}>
            {!bRunning && !bDone && <button type="button" onClick={()=>{primeAudio();bStart();}} className="timer-glass-btn-pri" style={BTN}>Start</button>}
            {bRunning && <>
              <button type="button" onClick={()=>setBRunning(r=>!r)} className="timer-glass-btn-pri" style={{...BTN,padding:"12px 28px"}}>Pause</button>
              <button type="button" onClick={bReset} className="timer-glass-btn-ghost" style={BTN_GHOST}>Reset</button>
            </>}
            {bDone && <button type="button" onClick={bReset} className="timer-glass-btn-pri" style={BTN}>Again</button>}
          </div>
          <div style={(!bRunning && !bDone) ? TIMER_BTN_SPACER_BELOW_IDLE : TIMER_BTN_SPACER_BELOW} aria-hidden />
        </div>)}

        </div>
      </div>
      <TabBar view={view} setView={setView} hasActiveSession={hasActiveSession} theme={theme} nightMode={nightMode} />
    </div>
  );
}
  ...
  return (
    <div>
      {!revealed ? (
        ...
        <a href="mailto:hello@adamlorber.com" style={{
...
function WorkoutApp({ theme, toggleTheme, nightMode = false, toggleNight = () => {}, onGuidedOpen }) {
  ...
  const [exerciseDuration, setExerciseDuration] = useState(() => {
    const v = storageGet("axis_exercise_duration", 45);
    return [30, 45, 60].includes(Number(v)) ? Number(v) : 45;
  });
  ...
  useEffect(() => {
    if (view === "session") storageSet("axis_last_session_tab", tab);
  }, [tab, view]);
  ...
  const defaultDurationStr = (TRACKS[track] && TRACKS[track].duration) || "";
  const defaultMinutes = Number((defaultDurationStr.match(/\d+/) || ["0"])[0]) || 0;
  const fullSecondsFromLabel = defaultMinutes * 60;
  const sessionSeconds = computeSessionSeconds(totalCount, fullSecondsFromLabel, activeCount);
  const sessionMinutes = Math.round(sessionSeconds / 60) || 0;
  const defaultDurationLabel = (TRACKS[track] && TRACKS[track].duration) || `${sessionMinutes} min`;
  ...
  const toggle = id => {
    setDone(d => {
      ...
      if (next[id] && !todayLogged) {
        ...
        const newEntry = { date: today, track: (TRACKS[track] && TRACKS[track].label) || track, duration: Math.round(sessionSecs / 60) };
  ...
  const tabSubtitle = tab === "list"
    ? "See all moves and targets."
    : tab === "guided"
      ? "Step-by-step walkthrough."
      : "Your progress over time.";

  const SunIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="2" x2="12" y2="4"/><line x1="12" y1="20" x2="12" y2="22"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="2" y1="12" x2="4" y2="12"/><line x1="20" y1="12" x2="22" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>;
  const MoonIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>;
  ...
  if (view === "timer") {
    return <TimerView key="timer" theme={theme} view={view} setView={setView} hasActiveSession={hasActiveSession} css={css} activePeriod={activePeriod} nightMode={nightMode} />;
  }
  if (view === "settings") return (
    <div key="settings" className="app" data-theme={theme} data-night={nightMode ? "true" : "false"}>
      <style>{css}</style>
      <div className="app-orbs" style={{background:"var(--orb1), var(--orb2), var(--orb3)", transition:"background 0.4s ease"}} />
      <div className="app-body">
      <div className="app-page-header">
        <div className="app-label" style={{fontSize:24,fontWeight:600,textTransform:"uppercase",color:"var(--text-secondary)",marginBottom:20}}>AXIS</div>
        <div style={{fontSize:"30px",fontWeight:500,letterSpacing:"-0.02em",color:"var(--text-white)",marginBottom:36}}>Settings</div>
        <div style={{borderTop:"1px solid var(--border)",paddingTop:24,marginBottom:32}}>
          <div style={{fontSize:"13px",letterSpacing:"0.22em",textTransform:"uppercase",color:"var(--text-dimmer)",marginBottom:20}}>Session Bar</div>
          {[
            { label: "Show % done", desc: "Percentage of exercises completed in the current track.", val: showPct, set: setShowPct },
          ].map(({ label, desc, val, set }) => (
            <div key={label} style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:24,marginBottom:24}}>
              <div>
                <div style={{fontSize:"14px",fontWeight:500,color:"var(--text-white)",marginBottom:4}}>{label}</div>
                <div style={{fontSize:"12px",color:"var(--text-secondary)",lineHeight:1.45,maxWidth:280}}>{desc}</div>
              </div>
              <button onClick={() => set(s => !s)} style={{flexShrink:0,marginTop:2,width:44,height:26,border:"1px solid",borderColor:val?"var(--accent)":"var(--glass-border-strong)",borderRadius:13,background:val?"var(--accent)":"var(--glass-bg)",cursor:"pointer",position:"relative",transition:"all 0.2s"}}>
                <div style={{position:"absolute",top:3,left:val?22:3,width:18,height:18,borderRadius:"50%",background: nightMode ? "#000000" : "rgba(255,255,255,0.95)",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.3)"}}/>
              </button>
            </div>
          ))}
        </div>
        <div style={{borderTop:"1px solid var(--border)",paddingTop:24,marginBottom:32}}>
          <div style={{fontSize:"13px",letterSpacing:"0.22em",textTransform:"uppercase",color:"var(--text-dimmer)",marginBottom:16}}>Session</div>
          <div>
            <div style={{fontSize:"14px",fontWeight:500,color:"var(--text-white)",marginBottom:8}}>Exercise duration (LIST)</div>
            <div style={{fontSize:"12px",color:"var(--text-secondary)",lineHeight:1.45,marginBottom:12,maxWidth:280}}>Timer length for each exercise in LIST view.</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
              {[30, 45, 60].map(sec => (
                <button key={sec} onClick={() => setExerciseDuration(sec)} style={{fontFamily:"'Barlow',sans-serif",fontSize:12,letterSpacing:"0.06em",textTransform:"uppercase",padding:"8px 14px",borderRadius:8,border:"1px solid",borderColor:exerciseDuration === sec ? "var(--accent)" : "var(--glass-border)",background:exerciseDuration === sec ? "var(--accent)" : "var(--glass-bg)",color:exerciseDuration === sec ? (nightMode ? "#000" : "#fff") : "var(--text-secondary)",cursor:"pointer",fontWeight:600}}>
        ...
        <div style={{borderTop:"1px solid var(--border)",paddingTop:24,marginBottom:0}}>
          <div style={{fontSize:"13px",letterSpacing:"0.22em",textTransform:"uppercase",color:"var(--text-dimmer)",marginBottom:16}}>Data Management</div>
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>
            ...
            }} style={{background:"var(--glass-bg)",border:"1px solid var(--glass-border)",fontFamily:"'Barlow',sans-serif",fontSize:"13px",letterSpacing:"0.14em",textTransform:"uppercase",color:"var(--text-secondary)",cursor:"pointer",padding:"11px 18px",borderRadius:10,textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
            ...
            <label style={{background:"var(--glass-bg)",border:"1px solid var(--glass-border)",fontFamily:"'Barlow',sans-serif",fontSize:"13px",letterSpacing:"0.14em",textTransform:"uppercase",color:"var(--text-secondary)",cursor:"pointer",padding:"11px 18px",borderRadius:10,textAlign:"left",display:"flex",alignItems:"center",gap:10}}>
          ...
          <div style={{fontSize:"13px",letterSpacing:"0.22em",textTransform:"uppercase",color:"var(--text-dimmer)",marginBottom:16}}>About</div>
          <button onClick={() => { storageSet("axis_onboarded", false); window.location.reload(); }} style={{background:"none",border:"1px solid var(--glass-border)",fontFamily:"'Barlow',sans-serif",fontSize:"13px",letterSpacing:"0.16em",textTransform:"uppercase",color:"var(--text-dim)",cursor:"pointer",padding:"10px 18px",marginBottom:16,borderRadius:10}}>View Onboarding Again</button>
          <div>
            <FeedbackButton />
          </div>
          </div>
  ...
  if (view === "mood") {
    const MOOD_PERIODS = [
      { p:"dawn",    label:"Rise",    time:"5–11am",   desc:"Warm amber. Morning energy.",
        icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 16 A 8 8 0 0 1 20 16"/><line x1="0" y1="16" x2="24" y2="16"/></svg> },
      ...
      { p:"prime",   label:"Prime",   time:"5–10pm",   desc:"Deep blue. Winding down.",
        icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg> },
      { p:"rest",    label:"Rest",    time:"10pm–5am", desc:"Soft violet. Calm and dim.",
        icon:<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 3l1.5 5.5 6 .5-4.5 4 1.5 5.5L12 16l-4.5 2 1.5-5.5-4.5-4 6-.5L12 3z"/></svg> },
    ];
    ...
    return (
      <div className="app" data-theme={theme} data-night={nightMode ? "true" : "false"}>
        <style>{css}</style>
        <div className="app-orbs" style={{background:"var(--orb1), var(--orb2), var(--orb3)", transition:"background 0.4s ease"}} />
        <div className="app-body">
          <div className="app-page-header">
            <div className="app-label" style={{fontSize:24,fontWeight:600,textTransform:"uppercase",color:"var(--text-secondary)",marginBottom:20}}>AXIS</div>
            <div style={{fontSize:"30px",fontWeight:500,letterSpacing:"-0.02em",color:"var(--text-white)",marginBottom:6}}>Mood</div>
            <div style={{fontSize:"14px",color:"var(--text-secondary)",marginBottom:24,lineHeight:1.45}}>
              Set the feeling of the app. Auto follows the time of day, or lock it to whatever matches how you feel right now.
            </div>

            <div style={{fontSize:12,letterSpacing:"0.20em",textTransform:"uppercase",color:"var(--text-dimmer)",marginBottom:10,fontWeight:500}}>Appearance</div>
            <div style={{...(nightMode ? { opacity: 0.45, pointerEvents: "none", transition: "opacity 0.25s ease" } : {})}}>
              <div style={{display:"flex",gap:10,marginBottom:20}}>
                {[{val:"dark",label:"Dark"},{val:"light",label:"Light"}].map(({val,label}) => (
                  <button key={val} onClick={nightMode ? undefined : toggleTheme} style={{
              ...
              {/* Auto toggle */}
              <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 18px",borderRadius:16,background:"var(--glass-bg)",border: activePeriod===null ? "1px solid var(--accent)" : "1px solid var(--glass-border)",marginBottom:12,transition:"all 0.2s",backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)"}}>
                <div>
                  <div style={{fontSize:"15px",fontWeight:500,color: activePeriod===null ? "var(--accent)" : "var(--text-white)",marginBottom:3}}>Auto</div>
                  <div style={{fontSize:"12px",color:"var(--text-secondary)"}}>
                    {activePeriod===null ? <span>On — currently <span style={{color:"var(--accent)",fontWeight:500}}>{nowPeriod}</span></span> : "Follows time of day"}
                  </div>
                </div>
                <button onClick={nightMode ? undefined : ()=>{ const n = activePeriod===null ? nowPeriod : null; setActivePeriod(n); storageSet("axis_period",n); }} style={{flexShrink:0,width:44,height:26,border:"1px solid",borderColor:activePeriod===null?"var(--accent)":"var(--glass-border-strong)",borderRadius:13,background:activePeriod===null?"var(--accent)":"var(--glass-bg)",cursor: nightMode ? "default" : "pointer",position:"relative",transition:"all 0.2s"}}>
                  <div style={{position:"absolute",top:3,left:activePeriod===null?22:3,width:18,height:18,borderRadius:"50%",background: nightMode ? "#000000" : "rgba(255,255,255,0.95)",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.3)"}}/>
                </button>
              </div>

              {/* Period cards — dull and unselectable when Night mode is on */}
              <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:32}}>
                {MOOD_PERIODS.map(({p, label, time, desc, icon}) => {
                  ...
                  return (
                    <button key={p} onClick={nightMode ? undefined : ()=>{ setActivePeriod(p); storageSet("axis_period",p); }} style={{
                      ...
                      <div style={{flex:1, minWidth:0}}>
                        <div style={{display:"flex",alignItems:"baseline",gap:8,marginBottom:3}}>
                          <span style={{fontSize:"16px",fontWeight:500,color: isSelected ? ct.accent : "var(--text-white)",fontFamily:"'Barlow',sans-serif",letterSpacing:"-0.01em"}}>{label}</span>
                          <span style={{fontSize:"12px",color:"var(--text-dimmer)"}}>{time}</span>
                          {isNow && !nightMode && <span style={{fontSize:"10px",letterSpacing:"0.16em",textTransform:"uppercase",color:"var(--accent)",opacity:0.7}}>now</span>}
                        </div>
                        <div style={{fontSize:"13px",color:"var(--text-secondary)"}}>{desc}</div>
                      </div>
                      {isSelected && !nightMode && <div style={{width:8,height:8,borderRadius:"50%",background:ct.accent,flexShrink:0,boxShadow:`0 0 8px ${ct.accentGlow}`}}/>}
                    </button>
                  );
                })}
              </div>
            ...
            {/* Night mode — at bottom as advanced option */}
            <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",gap:24,marginBottom:32}}>
              <div>
                <div style={{fontSize:"14px",fontWeight:500,color:"var(--text-white)",marginBottom:4}}>Night mode</div>
                <div style={{fontSize:"12px",color:"var(--text-secondary)",lineHeight:1.45,maxWidth:280}}>Pure black with vibrant red — like Apple Watch Ultra. For low-light use.</div>
              </div>
              <button onClick={toggleNight} style={{flexShrink:0,marginTop:2,width:44,height:26,border:"1px solid",borderColor:nightMode?"var(--accent)":"var(--glass-border-strong)",borderRadius:13,background:nightMode?"var(--accent)":"var(--glass-bg)",cursor:"pointer",position:"relative",transition:"all 0.2s"}}>
                <div style={{position:"absolute",top:3,left:nightMode?22:3,width:18,height:18,borderRadius:"50%",background:nightMode?"#000":"rgba(255,255,255,0.95)",transition:"left 0.2s",boxShadow:"0 1px 4px rgba(0,0,0,0.3)"}}/>
              </button>
            </div>
          </div>
          <div className="tab-bar-spacer" />
  ...
  if (view === "home") {
    const nowPeriod = getCircadianPeriod();
    const usedPeriod = activePeriod || nowPeriod;
    const homeCt = CIRCADIAN_THEMES[usedPeriod][theme === "dark" ? "dark" : "light"];
    const homeHighlight = homeCt && homeCt.accent ? homeCt.accent : "var(--accent)";
    const perMove = exerciseDuration || 45;
    const getTrackDisplayDuration = (trackId) => {
      const sections = getSections(trackId);
      const allCount = (sections || []).reduce((n, sec) => n + (sec.exercises || []).length, 0);
      const activeCount = (sections || []).reduce((n, sec) => n + (sec.exercises || []).filter(e => !skipped[e.id]).length, 0);
      const defaultStr = (TRACKS[trackId] && TRACKS[trackId].duration) || "";
      const defaultMins = Number((defaultStr.match(/\d+/) || ["0"])[0]) || 0;
      const fullSecs = defaultMins * 60;
      const sessionSecs = allCount > 0 && fullSecs > 0 ? Math.round((fullSecs * activeCount) / allCount) : 0;
      const mins = Math.round(sessionSecs / 60) || 0;
      const defaultLabel = (TRACKS[trackId] && TRACKS[trackId].duration) || `${mins} min`;
      return activeCount === allCount ? defaultLabel : `${mins} min`;
    };
    ...
    const groups = [
      ...
      { label: "Targeted",    ids: ["desk", "screen", "hip", "back", "thoracic"] },
      { label: "Body Maintenance", ids: ["steady", "freely", "feet", "knee", "wrist"] },
      { label: "Mental Wellness", ids: ["anxiety", "stress"] },
      { label: "Travel", ids: ["transit", "travel"] },
      { label: "Sports", ids: ["walking", "running", "golf", "tennis"] },
    ];
    return (
      <div className="app" data-theme={theme} data-night={nightMode ? "true" : "false"}>
        <style>{css}</style>
      <div className="app-orbs" style={{background:"var(--orb1), var(--orb2), var(--orb3)", transition:"background 0.4s ease"}} />
        <div className="app-body">
        <div className="home">
          <div className="home-hdr app-page-header">
            <div className="home-brand">
              <div className="app-label" style={{fontSize:24,fontWeight:600,textTransform:"uppercase",color:"var(--text-secondary)",marginBottom:20}}>AXIS</div>
            </div>
            <div className="home-tagline">Listen to your body. Select your focus.</div>
          </div>
          <div className="track-cards-outer">
            {/* Today block: time-of-day + suggestion + primary CTA */}
            <div className="track-group start-here-group" role="region" aria-label="Today's suggestion" style={{
              marginBottom:28, padding:"20px 20px 22px", borderRadius:24,
              background: homeHighlight, border:`2px solid ${homeHighlight}`,
              backdropFilter:"blur(8px) saturate(1.05)", WebkitBackdropFilter:"blur(8px) saturate(1.05)",
              boxShadow:`0 12px 32px ${homeHighlight}33, inset 0 1px 0 rgba(255,255,255,0.05)`,
            }}>
              <div style={{letterSpacing:"0.22em",textTransform:"uppercase",color:"rgba(255,255,255,0.92)",marginBottom:8,fontWeight:600}}>Today</div>
              <div style={{fontWeight:600,color:"rgba(255,255,255,0.98)",marginBottom:6}}>{periodLabels[nowPeriod]}</div>
              <div style={{color:"rgba(255,255,255,0.92)",marginBottom:6,lineHeight:1.4}}>{todaySug.suggestion}</div>
              <div style={{color:"rgba(255,255,255,0.86)",letterSpacing:"0.06em",marginBottom:18}}>{getTrackDisplayDuration(todayTrack.id)}</div>
              <button className="begin-btn" onClick={() => beginTrack(todayTrack.id)} style={{width:"100%",padding:"14px 24px",fontSize:15, background:"#ffffff", color: homeHighlight, border:"none", fontWeight:700}} aria-label={`Start ${todayTrack.label}, ${getTrackDisplayDuration(todayTrack.id)}`}>
                {todayTrack.label}
              </button>
            </div>
            <div style={{fontSize:11,letterSpacing:"0.22em",textTransform:"uppercase",color:"var(--accent)",marginBottom:8,fontWeight:600}}>Browse</div>
            {groups.map(g => (
              <div className="track-group" key={g.label}>
                <div className="track-group-label">{g.label}</div>
                <div className="track-cards">
                  {g.ids.map(id => { const t = TRACKS[id]; const cardDur = getTrackDisplayDuration(t.id); return (
                    <div className="track-card" key={t.id} onClick={() => beginTrack(t.id)} role="button" tabIndex={0} onKeyDown={e => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); beginTrack(t.id); } }} aria-label={`${t.label}, ${cardDur}. Begin session`}>
                      <div className="track-card-left">
                        <div className="track-card-name">{t.label}</div>
                        <div className="track-card-sub"><span className="glass-label">{t.subtitle}</span></div>
                        <div className="track-card-purpose">{t.purpose}</div>
                      </div>
                      <div className="track-card-right">
                        <div className="track-card-dur">{cardDur}</div>
                        <button className="begin-btn" onClick={e => { e.stopPropagation(); beginTrack(t.id); }} aria-label={`Begin ${t.label}`}>Begin</button>
                      </div>
                    </div>
                  );})}
                </div>
  ...
  // SESSION — empty state: spec — #0B121C bg, Inter, CTA pill (#1A232E + #5EEAD4), logo, tagline
      ...
      <div className="app" data-theme={theme} data-night={nightMode ? "true" : "false"}>
        ...
        <div
          ...
          style={{
            ...
            minHeight: "100vh",
            minHeight: "100dvh",
            padding: "calc(25vh + 48px) 32px 80px 48px",
            textAlign: "center",
            background: "#0B121C",
        ...
        >
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", width: "100%" }}>
            <div style={{ fontSize: 14, fontWeight: 300, color: "#ffffff", marginBottom: 20, textAlign: "center" }}>
              Ready for movement.
            </div>
            <button
              className="begin-btn session-browse-btn"
              onClick={() => setView("home")}
              style={{
                padding: "12px 48px",
                fontFamily: "'Barlow', sans-serif",
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
            <img
              src={theme === "dark" ? AXIS_LOGO_DARK : AXIS_LOGO_LIGHT}
              alt="AXIS"
              style={{ height: 72, width: "auto", objectFit: "contain", opacity: 0.3, marginBottom: 48 }}
            />
            <div
              style={{
                fontFamily: "'Barlow', sans-serif",
  ...
  const guidedOverlayEl = guidedActive ? (
    <GuidedOverlay
  ...
  return (
    ...
    <div className="app" data-theme={theme} data-night={nightMode ? "true" : "false"}>
      ...
      <div className="hdr-wrap">
        <div className="hdr">
          <button className="back-btn" onClick={goHome}>← All Workouts</button>
          <div className="hdr-row">
            <div className="hdr-left">
              <div className="hdr-title">{TRACKS[track].label}</div>
              <div className="hdr-meta">
                <div className="hdr-subtitle"><span className="glass-label">{TRACKS[track].subtitle}</span></div>
                <div className="hdr-dur">{sessionDurationLabel}</div>
              </div>
          ...
          <div className="tabs">
            {["list", "guided", "summary"].map(t => (
              <button
                key={t}
                className={`tab ${tab === t ? "on" : ""}`}
                onClick={() => setTab(t)}
              >
                {t === "guided" ? "Guided" : t.charAt(0).toUpperCase() + t.slice(1)}
              </button>
            ))}
          </div>
        </div>
      ...
      <div className="content" ref={sessionContentRef} onScroll={e => {
        if (tab === "list") {
          listScrollRef.current = e.currentTarget.scrollTop || 0;
        }
      }}>
        {/* Sub-bar: description + progress — sticky, bleeds to edges */}
        <div className="tab-subbar">
          <div className="tab-desc-line">
            {tab === "list" && "Your pace. Tap to expand, mark done when ready."}
            {tab === "guided" && "Hands-free. Auto-timed; audio cues coming soon."}
            {tab === "summary" && "Your progress over time."}
          </div>
          {tab !== "summary" && (
            <>
              <div className="tab-prog-row">
                <span style={{fontSize:11,color:"var(--text-dim)",letterSpacing:"0.04em"}}>
                  <span style={{fontWeight:700,color:"var(--accent)"}}>{totalDone}</span>
                  <span style={{color:"var(--text-dimmer)"}}> / {TOTAL} exercises</span>
                </span>
                <span style={{fontSize:11,color:"var(--text-dimmer)",letterSpacing:"0.08em"}}>{pct}%</span>
              </div>
              <div className="prog-bar" style={{marginBottom:6}}><div className="prog-fill" style={{ width: `${pct}%` }} /></div>
            </>
          )}
        </div>

        {/* Guided CTA — sits above Favorites/Skipped/Reset when GUIDED tab is active */}
        {tab === "guided" && (
          <div style={{padding:"14px 0 6px"}}>
            ...
            >
              ...
              <div style={{fontSize:11, fontWeight:400, letterSpacing:"0.12em", opacity:0.65, marginTop:4}}>
                {filteredAll.length} exercises · auto-timed
              </div>
            </button>
          </div>
        )}
        ...
        <div className="fav-filter">
          <button className={`fav-toggle ${showFavOnly ? "on" : ""}`} onClick={() => { setShowFavOnly(f => !f); setShowSkipOnly(false); }}>★ Favorites</button>
          <button className={`fav-toggle ${showSkipOnly ? "on" : ""}`} onClick={() => { setShowSkipOnly(f => !f); setShowFavOnly(false); }}>— Skipped</button>
          <button className="fav-toggle reset-pill" onClick={() => {
        ...
        {tab === "list" && (
          <>
            {(() => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {flat.length} moves · {perMoveSeconds} seconds each
                  </div>
                </div>
              );
            })()}
            {visibleSections.map(sec => (
              <div className="sg" key={sec.label}>
                <div className="sh">
                  <span className="sh-tag">{sec.tag}</span>
                  <span className="sh-name">{sec.label}</span>
                  <span className="sh-ct">{sec.exercises.filter(e => done[e.id]).length}/{sec.exercises.length}</span>
                </div>
                {sec.purpose && (
                  <div className="purpose-note">
                    <div className="purpose-label">Purpose</div>
                    <div className="purpose-text">{sec.purpose}</div>
                  </div>
                )}
            ...
            <div style={{marginTop:24, padding:"4px 0 8px"}}>
              <button
                type="button"
                onClick={() => { if (filteredAll.length > 0) { primeAudio(); setTab("guided"); setGuidedActive(true); } }}
                disabled={filteredAll.length === 0}
                style={{
          ...
          <>
            <div className="summary" style={{ marginTop: 24 }}>
              ...
              <div className="summary-grid">
                <div className="summary-cell"><div className="summary-num">{weekSessions.length}</div><div className="summary-lbl">Sessions</div></div>
                <div className="summary-cell"><div className="summary-num">{streak}</div><div className="summary-lbl">Day Streak</div></div>
                <div className="summary-cell"><div className="summary-num">{weekMinutes}<span style={{ fontSize: 14 }}>m</span></div><div className="summary-lbl">Minutes</div></div>
              </div>
            </div>
            <MomentumMap history={history} />
            {history.length > 0 && (
              <div className="history" style={{ marginTop: 24 }}>
                <div className="summary-title" style={{ marginBottom: 8 }}>Recent Sessions</div>
                {history.slice(0, 10).map((e, i) => (
                  <div className="history-row" key={i}>
                    <div><div className="history-date">{e.date}</div><div className="history-track">{e.track}</div></div>
                    <div className="history-detail">{e.duration}m</div>
                  </div>
                ))}
              </div>
            )}
            {history.length === 0 && (
              <div style={{ textAlign: "center", padding: "48px 0", color: "var(--text-dim)", fontSize: 12, letterSpacing: "0.12em", textTransform: "uppercase" }}>
                Complete a session to see your history
              </div>
            )}
            {/* Breathing bridge for mental wellness tracks */}
            {(track === "anxiety" || track === "stress") && totalDone > 0 && (
              <div style={{marginTop:28,padding:"18px",borderRadius:16,border:"1px solid var(--glass-border)",background:"var(--glass-bg)"}}>
                <div style={{fontSize:11,letterSpacing:"0.18em",textTransform:"uppercase",color:"var(--accent)",marginBottom:8}}>Continue the Reset</div>
...
function App() {
  ...
  const [justActivated, setJustActivated] = useState(false); // true immediately after finishing onboarding
  ...
  const complete = (payload) => {
    if (payload?.theme != null) setTheme(payload.theme);
    storageSet("axis_onboarded", true);
    // Mark onboarding complete and trigger a one-time blur+fade reveal into the main app
    setOnboarded(true);
    setJustActivated(true);
  };

  // After onboarding finishes, ease from blurred/transparent → sharp/opaque once per session
  useEffect(() => {
  ...
  // Guided overlay state lives here — outside WorkoutApp/.app — so position:fixed is never trapped
  const SunIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="5"/><line x1="12" y1="2" x2="12" y2="4"/><line x1="12" y1="20" x2="12" y2="22"/><line x1="4.22" y1="4.22" x2="5.64" y2="5.64"/><line x1="18.36" y1="18.36" x2="19.78" y2="19.78"/><line x1="2" y1="12" x2="4" y2="12"/><line x1="20" y1="12" x2="22" y2="12"/><line x1="4.22" y1="19.78" x2="5.64" y2="18.36"/><line x1="18.36" y1="5.64" x2="19.78" y2="4.22"/></svg>;
  const MoonIcon = () => <svg width="14" height="14" viewBox="0 0 24 24" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>;
  ...
  const mainInnerStyle = {
    filter: justActivated ? "blur(16px)" : "blur(0px)",
    opacity: justActivated ? 0 : 1,
    transform: justActivated ? "scale(0.96)" : "scale(1)",
    transition: "filter 600ms ease, opacity 600ms ease, transform 600ms ease",
    willChange: "filter, opacity, transform",
  };

  // If an external onboarding module is available, prefer it.
  try {
    if (typeof window !== 'undefined' && window.AXIS_Onboarding) {
      // Override the Onboarding symbol to point to the external implementation.
      Onboarding = window.AXIS_Onboarding;
    } else if (typeof Onboarding_orig !== 'undefined') {
      Onboarding = Onboarding_orig;
    }
  } catch (e) {}
      ...
      <div style={appShellStyle}>
        <div style={appContentStyle}>
          <div style={{ position: "relative", minHeight: "100dvh" }}>
            <Onboarding theme={theme} onComplete={complete} />
          </div>
        </div>
      </div>
    );
  ...
  // Main app: when coming directly from onboarding in this session, start slightly blurred/faded
  // and ease into full clarity for a subtle "step into the app" reveal.
  return (
    <div style={appShellStyle}>
      <div style={appContentStyle}>
...
function EolMarker() {
  ...
  return (
    ...
    >
      <span className="eol-xs">X X X X</span>
      <span className="eol-tag">X MOVE X MEND X MAINTAIN X</span>
    </div>
  );
}
    ...
    // Try to synchronously load axis_data.json so the inline app can initialize from it.
    // Synchronous XHR is used here intentionally to ensure compatibility with the existing inline script initialization.
    (function(){
      try {
        var xhr = new XMLHttpRequest();
        xhr.open('GET', './axis_data.json', false); // synchronous
        xhr.send(null);
        if (xhr.status === 200) {
          try {
            window.AXIS_JSON = JSON.parse(xhr.responseText);
            console.log('AXIS: loaded axis_data.json');
          } catch(e) { console.error('AXIS: parse axis_data.json failed', e); }
        } else {
          // not found — no-op, app will fall back to inline data
          // console.log('AXIS: axis_data.json not present (status ' + xhr.status + ')');
        }
      } catch(e) {
        // network errors or cross-origin — silently ignore and fallback to inline data
        console.error('AXIS: axis_data.json fetch failed', e);
      }
    })();
  

    // Mount point: attempt immediate mount; if not ready, poll briefly and listen for events.
    (function(){
      function tryMount(){
        try {
          if (typeof React === 'undefined' || typeof ReactDOM === 'undefined') return false;
          const rootEl = document.getElementById('root');
          if (!rootEl) return false;
          const root = ReactDOM.createRoot(rootEl);
          if (window && window.AXIS_Onboarding) {
            root.render(React.createElement(window.AXIS_Onboarding, {}));
            console.log('Mounted window.AXIS_Onboarding into #root');
            return true;
          } else if (window && window.App) {
            root.render(React.createElement(window.App));
            console.log('Mounted window.App into #root');
            return true;
          }
          return false;
        } catch (e) {
      ...
      // Immediate attempt
      if (!tryMount()) {
        // Poll briefly in case scripts load shortly after
        let tries = 0;
        const id = setInterval(() => {
          tries++;
          if (tryMount() || tries > 40) clearInterval(id);
        }, 200);
